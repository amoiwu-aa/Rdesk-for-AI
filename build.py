...
osx = platform.platform().startswith("Darwin") or platform.platform().startswith(
    "macOS"
)
hbb_name = "rdesk" + (".exe" if windows else "")
...
def setup_windows_bindgen_env():
    if not windows:
        return

    def ensure_msvc_env():
        if os.environ.get("INCLUDE") and os.environ.get("VCINSTALLDIR"):
            return
        ...
        vcvarsall = (
            Path(installation_path) / "VC" / "Auxiliary" / "Build" / "vcvarsall.bat"
        )
        if not vcvarsall.exists():
            return

        dump_env_cmd = f'call "{vcvarsall}" x64 >nul && set'
        try:
            output = subprocess.check_output(dump_env_cmd, text=True, shell=True)
        except Exception:
            return
    ...
    vcpkg_installed_root = os.environ.get("VCPKG_INSTALLED_ROOT")
    if vcpkg_installed_root:
        include_dir = Path(vcpkg_installed_root) / "x64-windows-static" / "include"
        if include_dir.exists():
            include_dirs.append(str(include_dir))

    include_env = os.environ.get("INCLUDE", "")
    if include_env:
        include_dirs.extend([p for p in include_env.split(";") if p])
    ...
    def to_clang_arg(path: str) -> str:
        return f'-I"{path}"' if " " in path else f"-I{path}"

    auto_args = " ".join(to_clang_arg(p) for p in deduped)
    existing = os.environ.get("BINDGEN_EXTRA_CLANG_ARGS", "").strip()
    merged = " ".join(x for x in [existing, auto_args] if x).strip()
    os.environ["BINDGEN_EXTRA_CLANG_ARGS"] = merged
    os.environ["BINDGEN_EXTRA_CLANG_ARGS_x86_64-pc-windows-msvc"] = merged

    cpath_existing = os.environ.get("CPATH", "")
    include_joined = ";".join(deduped)
    os.environ["CPATH"] = ";".join(x for x in [cpath_existing, include_joined] if x)


def setup_windows_flutter_env():
    if not windows:
        return

    current_path = os.environ.get("PATH", "")
    for candidate in current_path.split(";"):
        if not candidate:
            continue
        flutter_bat = Path(candidate) / "flutter.bat"
        if flutter_bat.exists():
            return

    candidates = [
        Path(r"C:\Users\wuxiang\flutter\bin"),
        Path(r"D:\rustdesk\flutter\bin"),
    ]

    user_profile = os.environ.get("USERPROFILE")
    if user_profile:
        candidates.append(Path(user_profile) / "flutter" / "bin")

    for candidate in candidates:
        if (candidate / "flutter.bat").exists():
            os.environ["PATH"] = (
                f"{candidate};{current_path}" if current_path else str(candidate)
            )
            return
...
def make_parser():
    ...
    parser.add_argument(
        "--flutter", action="store_true", help="Build flutter package", default=False
    )
    parser.add_argument(
        "--hwcodec",
        action="store_true",
        help="Enable feature hwcodec" + ("" if windows or osx else ", need libva-dev."),
    )
    parser.add_argument(
        "--vram",
        action="store_true",
    ...
    parser.add_argument(
        "--av1",
        action="store_true",
        help="Enable feature av1",
    )
    parser.add_argument(
        "--portable", action="store_true", help="Build windows portable"
    )
    parser.add_argument(
...
def download_extract_features(features, res_dir):
    ...
    for feat, feat_info in features.items():
        includes = (
            feat_info["include"]
            if "include" in feat_info and feat_info["include"]
            else []
        )
        includes = [re.compile(p) for p in includes]
        excludes = (
            feat_info["exclude"]
            if "exclude" in feat_info and feat_info["exclude"]
            else []
        )
        ...
        for line in checksum_md5_response.read().decode("utf-8").splitlines():
            if line.split()[1] == download_filename:
                ...
                filename, _headers = urllib.request.urlretrieve(
                    feat_info["zip_url"], download_filename
                )
...
def external_resources(flutter, args, res_dir):
    ...
    print(f"Build with features {feature_names}")
...
def build_flutter_dmg(version, features):
    ...
    system2(
        "cp -rf ../target/release/service ./build/macos/Build/Products/Release/RDesk.app/Contents/MacOS/"
    )
    """
    system2(
        "create-dmg --volname \"RDesk Installer\" --window-pos 200 120 --window-size 800 400 --icon-size 100 --app-drop-link 600 185 --icon RDesk.app 200 190 --hide-extension RDesk.app rdesk.dmg ./build/macos/Build/Products/Release/RDesk.app")
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f"cargo build --features {features} --lib --release")
        if not os.path.exists("target/release/librdesk.dll"):
    ...
    system2(
        f"python ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/rdesk.exe"
    )
    os.chdir("../..")
    if os.path.exists("./rdesk_portable.exe"):
        os.replace(
            "./target/release/rustdesk-portable-packer.exe", "./rdesk_portable.exe"
        )
    else:
        os.rename(
            "./target/release/rustdesk-portable-packer.exe", "./rdesk_portable.exe"
        )
    print(f"output location: {os.path.abspath(os.curdir)}/rdesk_portable.exe")
    os.replace("./rdesk_portable.exe", f"./rdesk-{version}-install.exe")
    print(f"output location: {os.path.abspath(os.curdir)}/rdesk-{version}-install.exe")
...
def main():
    ...
    setup_windows_bindgen_env()
    setup_windows_flutter_env()
    ...
    if windows:
        ...
        system2("mv target/release/rdesk.exe target/release/RDesk.exe")
        pa = os.environ.get("P")
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f"signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  "
                "target\\release\\rdesk.exe"
            )
        else:
            print("Not signed")
        system2(f"cp -rf target/release/RDesk.exe {res_dir}")
        os.chdir("libs/portable")
        system2("pip3 install -r requirements.txt")
        system2(
        ...
        if flutter:
            ...
            if osx:
                system2(
                    "strip target/release/bundle/osx/RDesk.app/Contents/MacOS/rustdesk"
                )
                system2(
                    "cp libsciter.dylib target/release/bundle/osx/RDesk.app/Contents/MacOS/"
                )
    ...
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RDesk.app/Contents/MacOS/rustdesk
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/rustdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/RDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RDesk.app/Contents/MacOS/*
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/RDesk.app
    """.format(pa)
                    )
                system2(
                    'create-dmg "RDesk %s.dmg" "target/release/bundle/osx/RDesk.app"'
                    % version
                )
                os.rename("RDesk %s.dmg" % version, "rustdesk-%s.dmg" % version)
                if pa:
                    system2(
                        """
    ...
    # verify:  spctl -a -t exec -v /Applications/RDesk.app
    """.format(pa, version)
                    )
                else:
                    print("Not signed")
