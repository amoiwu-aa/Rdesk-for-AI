    ...
    unsafe {
        {
            ...
            if let Some(parent) = abs_path.parent() {
                let cer_path = parent.join("RustDeskIddDriver.cer");
                if cer_path.exists() {
                    use std::os::windows::process::CommandExt;
                    let cer_str = cer_path.to_string_lossy().to_string();
                    // Self-signed WDK test cert needs both Root (as CA) and TrustedPublisher
                    for store in &["Root", "TrustedPublisher"] {
                        let r = std::process::Command::new("certutil")
                            .creation_flags(0x08000000) // CREATE_NO_WINDOW
                            .args(&["-addstore", store, &cer_str])
                            .output();
                        if let Err(e) = r {
                            hbb_common::log::error!("certutil -addstore {} failed: {}", store, e);
                        }
                    }
                }
            }
