...
unsafe impl Send for VRamEncoder {}
...
impl EncoderApi for VRamEncoder {
    ...
    fn disable(&self) {
        log::warn!("VRamEncoder::disable called, ignoring (no software fallback available)");
    }
}
...
impl VRamEncoder {
    ...
    pub fn bitrate(fmt: DataFormat, width: usize, height: usize, ratio: f32) -> u32 {
        let mut b = crate::hwcodec::HwRamEncoder::calc_bitrate(width, height, ratio, fmt == DataFormat::H264);
        if fmt == DataFormat::AV1 {
            b = (b as f32 * 0.6) as u32;
        } else if fmt == DataFormat::H265 {
