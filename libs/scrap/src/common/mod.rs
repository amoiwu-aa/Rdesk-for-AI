...
#[cfg(all(dxgi, feature = "vram"))]
pub use self::dxgi::OwnedTextureFrame;
...
pub fn would_block_if_equal(old: &mut Vec<u8>, b: &[u8]) -> std::io::Result<()> {
    // Use hash comparison instead of full byte-by-byte compare to save memory bandwidth
    let mut hasher = std::collections::hash_map::DefaultHasher::new();
    b.hash(&mut hasher);
    let new_hash = hasher.finish();

    // Store hash in the first 8 bytes of old, length check avoids false positive on first call
    if old.len() == 8 {
        let old_hash = u64::from_ne_bytes([
    ...
    old.resize(8, 0);
    old.copy_from_slice(&new_hash.to_ne_bytes());
    Ok(())
}
...
pub trait TraitCapturer {
    ...
    fn owned_texture_frame(
        &mut self,
        _timeout: std::time::Duration,
    ) -> std::io::Result<OwnedTextureFrame> {
...
pub enum EncodeInput<'a> {
    YUV(&'a [u8]),
    Texture((*mut c_void, usize)),
    BGRA(&'a [u8], usize, usize, usize), // data, width, height, stride
}
...
impl From<&VideoFrame> for CodecFormat {
    fn from(it: &VideoFrame) -> Self {
        match it.union {
            ...
            Some(video_frame::Union::Sccs(_)) => CodecFormat::ScreenContent,
...
impl From<&video_frame::Union> for CodecFormat {
    fn from(it: &video_frame::Union) -> Self {
        match it {
            ...
            video_frame::Union::Sccs(_) => CodecFormat::ScreenContent,
            _ => CodecFormat::Unknown,
        }
    }
}
...
impl From<&CodecName> for CodecFormat {
    fn from(value: &CodecName) -> Self {
        match value {
            ...
            CodecName::AV1RAM(_) | CodecName::AV1VRAM => Self::AV1,
            CodecName::SCC => Self::ScreenContent,
        }
    }
}
...
impl ToString for CodecFormat {
    fn to_string(&self) -> String {
        match self {
            ...
            CodecFormat::ScreenContent => "SCC".into(),
...
pub trait GoogleImage {
    fn width(&self) -> usize;
    fn height(&self) -> usize;
    fn stride(&self) -> [i32; 4];
    fn planes(&self) -> [*mut u8; 4];
    ...
    fn to(&self, rgb: &mut ImageRgb) {
        ...
        let total = rgb.h * bytes_per_row;
        rgb.raw.reserve(total);
        // SAFETY: libyuv FFI will overwrite all bytes; skip zero-fill
        unsafe {
            rgb.raw.set_len(total);
        }
