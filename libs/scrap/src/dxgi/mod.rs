...
#[derive(Clone)]
pub struct OwnedTextureFrame {
    texture: ComPtr<ID3D11Texture2D>,
    rotation: usize,
}

unsafe impl Send for OwnedTextureFrame {}
unsafe impl Sync for OwnedTextureFrame {}

impl OwnedTextureFrame {
    pub fn raw(&self) -> (*mut c_void, usize) {
        (self.texture.0 as *mut c_void, self.rotation)
    }
}
...
impl Capturer {
    ...
    pub fn owned_texture_frame(&mut self, timeout: UINT) -> io::Result<OwnedTextureFrame> {
        let (texture, rotation) = self.get_texture(timeout)?;
        if texture.is_null() {
            return Err(std::io::ErrorKind::WouldBlock.into());
        }
        let texture = unsafe {
            let ptr = texture as *mut ID3D11Texture2D;
            if !ptr.is_null() {
                (*(ptr as *mut IUnknown)).AddRef();
            }
            ComPtr(ptr)
        };
        Ok(OwnedTextureFrame { texture, rotation })
    }
