...
impl Client {
    ...
    async fn _start_inner(
        ...
        let mut peer_addr_v6 = std::net::SocketAddr::from(([0; 16], 0));
        ...
        if !key.is_empty() && !token.is_empty() {
            ...
            let secure_result = secure_tcp(&mut socket, &key).await;
            secure_result.map_err(|e| anyhow!("Failed to secure tcp: {}", e))?;
        ...
        for i in 1..=3 {
            ...
            let send_result = socket.send(&msg_out).await;
            send_result?;
            // below timeout should not bigger than hbbs's connection timeout.
            let msg_result = crate::get_next_nonkeyexchange_msg(&mut socket, Some(i * 3000)).await;
            if let Some(msg_in) = msg_result {
                match msg_in.union {
                    ...
                    Some(rendezvous_message::Union::RelayResponse(rr)) => {
                        ...
                        if let Some(s) = ipv6.0 {
                            ...
                            if addr.port() > 0 {
                                connect_futures
                                    .push(udp_nat_connect(s, "IPv6", addr, CONNECT_TIMEOUT).boxed());
                            }
                        }
    ...
    async fn connect(
        ...
        log::info!(
            "[net][connect] peer_addr={}, timeout_ms={}",
            peer,
            connect_timeout
        );
        log::info!(
            "[net][connect] context: peer_id={}, local_addr={}, relay_server={}, force_relay={}, direct_failures={}, is_local={}, peer_nat_type={:?}, my_nat_type={}, udp_nat={}, udp_v6={}",
            peer_id,
            local_addr,
            relay_server,
        ...
        if let Some(udp_socket_nat) = udp_socket_nat {
            connect_futures.push(udp_nat_connect(udp_socket_nat, "UDP", peer.clone(), connect_timeout).boxed());
        }
        if let Some(udp_socket_v6) = udp_socket_v6 {
            connect_futures.push(udp_nat_connect(udp_socket_v6, "IPv6", peer_addr_v6, connect_timeout).boxed());
        }
        // Run all connection attempts concurrently, return the first successful one
        let (mut conn, kcp, mut typ) = match select_ok(connect_futures).await {
            Ok(conn) => {
                log::info!(
                    "[net][connect] direct transport selected: peer_id={}, transport={}, elapsed={:?}",
                    peer_id,
                    conn.0 .2,
                    start.elapsed()
                );
            ...
            Err(e) => {
                log::warn!(
                    "[net][connect] all direct transports failed: peer_id={}, peer_addr={}, relay_server={}, err={}",
                    peer_id,
                    peer,
                    relay_server,
                    e
        ...
        if interface.is_force_relay() || conn.is_err() {
            if !relay_server.is_empty() {
                log::info!(
                    "[net][connect] falling back to relay: peer_id={}, reason={}, relay_server={}",
                    peer_id,
                ...
                if let Err(e) = conn {
                    ...
                    log::error!(
                        "[net][connect] relay connection failed: peer_id={}, relay_server={}, err={}",
                        peer_id,
                        relay_server,
                        e
                    );
        ...
        let mut conn = conn?;
        log::info!(
            "[net][connect] established: elapsed={:?}, transport={}, punch={}, peer_id={}, direct={}, local_addr={}",
            start.elapsed(),
            typ,
            punch_type,
            peer_id,
        ...
        log::debug!("[net][connect] secure_connection ok: punch={}", punch_type);
    ...
    #[inline]
    async fn sleep_before_relay_retry(attempt: usize) {
        let base_ms = match attempt {
            1 => 200,
            2 => 500,
            _ => 1_000,
        };
        let jitter_ms = (hbb_common::time_based_rand() % 150) as u64;
        let delay = base_ms + jitter_ms;
        log::debug!("[net][relay] retry after {}ms", delay);
    ...
    async fn request_relay(
        ...
        let mut ipv4 = true;
        let mut last_err = "Timeout".to_owned();
        ...
        for i in 1..=3 {
            ...
            let mut socket = match connect_tcp(rendezvous_server, CONNECT_TIMEOUT).await {
                Ok(socket) => socket,
                Err(err) => {
                    last_err = format!("Failed to connect to rendezvous server: {err}");
                    log::warn!(
                        "[net][relay] request connect failed: attempt={}, peer_id={}, rendezvous_server={}, err={}",
                        i,
                        peer,
                        rendezvous_server,
                        err
                    ...
                    if i < 3 {
                        Self::sleep_before_relay_retry(i).await;
                    }
                    continue;
                }
            };
            ...
            if !key.is_empty() && !token.is_empty() {
                ...
                if let Err(err) = secure_tcp(&mut socket, key).await {
                    last_err = format!("Failed to secure tcp: {err}");
                    log::warn!(
                        "[net][relay] request secure_tcp failed: attempt={}, peer_id={}, rendezvous_server={}, err={}",
                        i,
                        peer,
                        rendezvous_server,
                        err
            ...
            log::info!(
                "[net][relay] attempt={}, peer_id={}, uuid={}, relay_server={}, secure={}",
                i,
                peer,
                uuid,
                relay_server,
            ...
            if let Err(err) = socket.send(&msg_out).await {
                last_err = format!("Failed to request relay: {err}");
                log::warn!(
                    "[net][relay] request send failed: attempt={}, peer_id={}, relay_server={}, err={}",
                    i,
                    peer,
                    relay_server,
                    err
                ...
                crate::get_next_nonkeyexchange_msg(&mut socket, Some(CONNECT_TIMEOUT)).await
            {
                if let Some(rendezvous_message::Union::RelayResponse(rs)) = msg_in.union {
                    ...
                    log::info!(
                        "[net][relay] request accepted: attempt={}, peer_id={}, relay_server={}, uuid={}",
                        i,
                        peer,
                        relay_server,
                        uuid
                ...
                log::warn!(
                    "[net][relay] unexpected response message: attempt={}, peer_id={}, rendezvous_server={}",
                    i,
                    peer,
                    rendezvous_server
                );
            }
            last_err = "Timeout".to_owned();
            log::warn!(
                "[net][relay] request timed out: attempt={}, peer_id={}, rendezvous_server={}, relay_server={}",
                i,
                peer,
                rendezvous_server,
                relay_server
    ...
    async fn create_relay(
        ...
        let relay_endpoint = ipv4_to_ipv6(check_port(relay_server.clone(), RELAY_PORT), ipv4);
        log::info!(
            "[net][relay] connecting endpoint: peer_id={}, relay_server={}, endpoint={}, ipv4={}, conn_type={:?}",
            peer,
            relay_server,
            relay_endpoint,
            ipv4,
        ...
        let mut conn = connect_tcp(relay_endpoint, CONNECT_TIMEOUT)
            .await
        ...
        conn.send(&msg_out).await?;
        log::info!(
            "[net][relay] transport connected: peer_id={}, relay_server={}, local_addr={}",
            peer,
            relay_server,
            conn.local_addr()
        );
    ...
    fn build_output_stream<T: cpal::Sample + cpal::SizedSample + cpal::FromSample<f32>>(
        ...
        let stream = device.build_output_stream(
            ...
            move |data: &mut [T], _info: &cpal::OutputCallbackInfo| {
                ...
                #[cfg(not(target_os = "android"))]
                if having < n {
                    // Not enough samples available, output what we have instead of blocking
                    n = having;
                }
                #[cfg(target_os = "android")]
                if having < n {
...
impl LoginConfigHandler {
    ...
    pub fn initialize(
        ...
        // Reset performance-related settings to standard defaults.
        // Flutter UI always starts in standard mode (PerformanceModeState.init),
        // so backend should match to avoid stale game/office settings.
        let prev_mode = config
            .options
            .get("performance-mode")
            .cloned()
            .unwrap_or_default();
        if !prev_mode.is_empty() && prev_mode != "standard" {
            config.image_quality = "balanced".to_owned();
            config.custom_image_quality = vec![];
            config
                .options
                .insert("custom-fps".to_string(), "60".to_string());
        }
        config.options.remove("codec-preference");
        config.options.remove("performance-mode");

        // Must persist to disk, not just memory. Other methods like set_option()
        // call load_config() which reloads from disk, overwriting in-memory changes.
    ...
    fn get_option_message(&self, ignore_default: bool) -> Option<OptionMessage> {
        ...
        // Always initialize custom_fps, regardless of quality preset.
        // Previously this was only done for "custom" quality, causing fps_control to
        // default to 30fps for "best"/"low"/"balanced" presets.
        #[cfg(feature = "flutter")]
        {
            let custom_fps_val = if let Some(custom_fps) = self.options.get("custom-fps") {
                let mut fps = custom_fps.parse().unwrap_or(60);
                if !allow_more && fps > 120 {
                    fps = 120;
                }
                fps
            } else {
                // No custom-fps saved: default to 60
                60
            };
            msg.custom_fps = custom_fps_val;
            *self.custom_fps.lock().unwrap() = Some(custom_fps_val as _);
        }
    ...
    pub fn save_image_quality(&mut self, value: String) -> Option<Message> {
        ...
        config.custom_image_quality.clear();
        self.save_config(config);
        res
    }
                    ...
                    MediaData::VideoFrame(_) | MediaData::VideoQueue => {
                        let vf = match data {
                            MediaData::VideoFrame(vf) => {
                                discard_queue.store(false, Ordering::Release);
                                *vf
                            }
                            MediaData::VideoQueue => {
                                if let Some(vf) = video_queue.pop() {
                                    if discard_queue.load(Ordering::Acquire) {
                                        continue;
                                    }
                                    vf
                                } else {
                        ...
                        if video_handler.is_none() {
                            ...
                            *decoder_type.write().unwrap() =
                                handler.decoder.decoder_type().to_string();
                            log::info!(
                                "Video decoder created: type={}, format={:?}",
                                handler.decoder.decoder_type(),
                    ...
                    MediaData::Reset => {
                        if let Some(handler) = video_handler.as_mut() {
                            ...
                            *decoder_type.write().unwrap() =
                                handler.decoder.decoder_type().to_string();
                            log::info!(
                                "Video decoder reset: type={}, format={:?}",
                                handler.decoder.decoder_type(),
                                handler.decoder.format()
                            );
                        }
...
pub mod peer_online {
    ...
    async fn create_online_stream() -> ResultType<Stream> {
        ...
        hbb_common::socket_client::connect_tcp_local(online_server, None, CONNECT_TIMEOUT).await
    }
