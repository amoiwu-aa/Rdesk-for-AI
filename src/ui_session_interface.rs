...
impl<T: InvokeUiSession> Session<T> {
    ...
    pub fn save_image_quality(&self, value: String) {
        ...
        // Let the user control FPS independently from image quality preset.
        // Previously this capped non-custom quality to 60fps, blocking game mode 120fps.
    }
    ...
    pub fn alternative_codecs(&self) -> (bool, bool, bool, bool) {
        ...
        let scc = decoder.ability_scc > 0;
        let av1 = decoder.ability_av1 > 0;
    ...
    pub fn send_performance_mode(&self, mode: String) {
        let mut option = OptionMessage::new();
        option.performance_mode = mode;
        let mut misc = Misc::new();
    ...
    pub fn remove_port_forward(&self, port: i32) {
        // Just stop listening on the port.
        // We do not delete it from config so the UI can still show it and allow toggling.
        // In order to completely delete a port forward from config, the user will now use an explicit delete action (we could add delete_port_forward if needed, but for now we repurpose remove).
    ...
    pub fn add_port_forward(&self, port: i32, remote_host: String, remote_port: i32) {
        ...
        if remote_host == "#DELETE#" {
            config.port_forwards.retain(|x| x.0 != port);
        ...
        if config
            ...
            .any(|x| x.0 == port)
        {
            // Already contains this port. dispatch to session.
            self.send(Data::AddPortForward((port, remote_host, remote_port)));
            return;
        }
    ...
    pub fn set_option(&self, k: String, mut v: String) {
        ...
        lc.set_option(k.clone(), v.clone());
        drop(lc);

        // Send performance-mode to server as OptionMessage so the server
        // can set extreme-low-bandwidth and other service options.
        if k == "performance-mode" {
            self.send_performance_mode(v);
        }
    }
    ...
    *handler.sender.write().unwrap() = Some(sender.clone());
    ...
    if handler.is_port_forward() {
        if handler.is_rdp() {
            ...
            let pfs = handler.lc.read().unwrap().load_config().port_forwards;
            let mut queues = HashMap::<i32, mpsc::UnboundedSender<Data>>::new();
            for d in pfs {
                let disabled_key = format!("pf_disabled_{}", d.0);
                if handler.get_option(disabled_key) != "1" {
