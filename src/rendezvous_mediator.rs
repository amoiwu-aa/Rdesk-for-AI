...
lazy_static::lazy_static! {
    ...
    static ref UDP_FAIL_COUNTS: Mutex<HashMap<String, usize>> = Default::default();
}
...
impl RendezvousMediator {
    async fn udp_fail_count(host: &str) -> usize {
        *UDP_FAIL_COUNTS.lock().await.get(host).unwrap_or(&0)
    }

    async fn bump_udp_fail_count(host: &str) -> usize {
        let mut counts = UDP_FAIL_COUNTS.lock().await;
        let count = counts.entry(host.to_owned()).or_default();
        *count += 1;
        *count
    }

    async fn reset_udp_fail_count(host: &str) {
        UDP_FAIL_COUNTS.lock().await.remove(host);
    }
    ...
    pub async fn start_udp(server: ServerPtr, host: String) -> ResultType<()> {
        ...
        log::info!("[net][rz][udp] start: host={host}");
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    if timeout {
                        fails += 1;
                        log::warn!(
                            "[net][rz][udp] register timeout: host={}, fails={}, reg_timeout_ms={}",
                            host,
                            fails,
                            reg_timeout
                        );
                        if fails >= MAX_FAILS2 {
                            ...
                            old_latency = 0;
                                ...
                                if let Some((s, new_addr)) = socket_client::rebind_udp_for(&rz.host).await? {
                                    log::info!(
                                        "[net][rz][udp] socket rebound: host={}, new_addr={:?}",
                                        rz.host,
                                        new_addr
                                    );
                                    socket = s;
    ...
    pub async fn start_tcp(server: ServerPtr, host: String) -> ResultType<()> {
        let host = check_port(&host, RENDEZVOUS_PORT);
        log::info!(
            "[net][rz][tcp] start: host={}, endpoint={}, use_ws={}",
            host,
            hbb_common::websocket::check_ws(&host),
            use_ws()
        );
            ...
            select! {
                res = conn.next() => {
                    ...
                    if bytes.is_empty() {
                        ...
                        log::trace!("[net][rz][tcp] heartbeat received: host={}", host);
                ...
                _ = timer.tick() => {
                    ...
                    if last_recv_msg.elapsed().as_millis() as u64 > rz.keep_alive as u64 * 3 / 2 {
                        log::warn!(
                            "[net][rz][tcp] timeout: host={}, idle_ms={}, keep_alive_ms={}",
                            host,
                            last_recv_msg.elapsed().as_millis(),
                            rz.keep_alive
                        );
    ...
    pub async fn start(server: ServerPtr, host: String) -> ResultType<()> {
        let host = check_port(&host, RENDEZVOUS_PORT);
        log::info!("[net][rz] start mediator: host={}", host);
        ...
        {
            log::info!(
                "[net][rz] selecting tcp path: host={}, test_tcp={}, proxy={}, use_ws={}, udp_disabled={}",
                host,
        ...
        } else if Self::udp_fail_count(&host).await >= UDP_FAIL_THRESHOLD {
            // UDP failed too many times, auto fallback to WS
            log::info!(
                "[net][rz] udp failed {} times for {}, auto using WebSocket",
                Self::udp_fail_count(&host).await,
                host
            );
            let result = Self::start_tcp(server, host.clone()).await;
            // If WS also fails, reset counter to retry UDP next round
            if result.is_err() {
                Self::reset_udp_fail_count(&host).await;
            }
            result
        } else {
            let result = Self::start_udp(server.clone(), host.clone()).await;
            if result.is_err() {
                let count = Self::bump_udp_fail_count(&host).await;
                log::warn!(
                    "[net][rz] udp failed for {} ({}/{}), will fallback to WS if threshold reached",
                    host,
                    count,
                    UDP_FAIL_THRESHOLD
                );
    ...
    async fn register_pk(&mut self, socket: Sink<'_>) -> ResultType<()> {
        ...
        log::debug!(
            "[net][rz] register_pk sending: host={}, id={}, uuid_len={}",
            self.host,
            id,
            uuid.len()
        );
    ...
    async fn register_peer(&mut self, socket: Sink<'_>) -> ResultType<()> {
        ...
        log::debug!(
            "[net][rz] register_peer sending: host={}, id={}, serial={}",
            self.host,
            id,
            serial
        );
...
async fn udp_nat_listen(
    ...
    let socket_cloned = socket.clone();
    let func = async {
        let res = crate::punch_udp(socket.clone(), peer_addr, true).await?;
