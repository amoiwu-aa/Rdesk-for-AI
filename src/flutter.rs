...
impl InvokeUiSession for FlutterHandler {
    ...
    fn update_quality_status(&self, status: &QualityStatus) {
        ...
        self.push_event(
            ...
            &[
                ("speed", &status.speed.clone().unwrap_or(NULL.clone())),
                (
                    "fps",
                ...
                (
                    "decode_fps",
                    &status.decode_fps.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "decoder_type",
                    &status.decoder_type.clone().unwrap_or(NULL.clone()),
                ),
                (
                    "performance_mode",
                    &status.performance_mode.clone().unwrap_or(NULL.clone()),
                ),
                ("quality", &status.quality.clone().unwrap_or(NULL.clone())),
                (
                    "requested_fps",
                    &status.requested_fps.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "auto_adjust_fps",
                    &status.auto_adjust_fps.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "fps_limit",
                    &status.fps_limit.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "game_mode",
                    &status.game_mode.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "server_performance_mode",
                    &status.server_performance_mode.clone().unwrap_or(NULL.clone()),
                ),
                (
                    "server_game_mode",
                    &status.server_game_mode.map_or(NULL.clone(), |it| it.to_string()),
                ),
                (
                    "server_game_mode_reason",
                    &status.server_game_mode_reason.clone().unwrap_or(NULL.clone()),
                ),
                (
                    "server_qos_fps",
                    &status.server_qos_fps.map_or(NULL, |it| it.to_string()),
                ),
                (
                    "server_max_delay",
                    &status.server_max_delay.map_or(NULL, |it| it.to_string()),
                ),
                (
                    "server_response_delayed",
                    &status.server_response_delayed.map_or(NULL, |it| it.to_string()),
                ),
                (
                    "server_current_ratio",
                    &status.server_current_ratio.map_or(NULL, |it| format!("{:.2}", it)),
                ),
                (
                    "server_target_ratio",
                    &status.server_target_ratio.map_or(NULL, |it| format!("{:.2}", it)),
                ),
                (
                    "server_max_custom_fps",
                    &status.server_max_custom_fps.map_or(NULL, |it| it.to_string()),
                ),
                (
                    "server_abr",
                    &status.server_abr.map_or(NULL, |it| it.to_string()),
                ),
                ...
                ("chroma", &status.chroma.clone().unwrap_or(NULL.clone())),
            ],
            &[],
        );
    }
