...
const FILE_JOB_INTERVAL: Duration = Duration::from_millis(5);
const RELAXED_SESSION_TIMEOUT: Duration = Duration::from_secs(60);
...
pub struct Remote<T: InvokeUiSession> {
    ...
    quality_status_cache: QualityStatus,
    file_job_pump_running: bool,
}
...
impl<T: InvokeUiSession> Remote<T> {
    pub fn new(
        ...
        Self {
            ...
            quality_status_cache: Default::default(),
            file_job_pump_running: false,
        }
    }

    fn ensure_file_job_pump(&mut self) {
        if self.file_job_pump_running {
            return;
        }
        self.file_job_pump_running = true;
        let tx = self.sender.clone();
        tokio::spawn(async move {
            let mut interval = time::interval(FILE_JOB_INTERVAL);
            loop {
                interval.tick().await;
                if tx.send(Data::FileJobTick).is_err() {
                    break;
                }
            }
        });
    ...
    pub async fn io_loop(&mut self, key: &str, token: &str, round: u32) {
        ...
        .await
        {
            Ok(((mut peer, direct, pk, kcp, stream_type), (feedback, rendezvous_server))) => {
                let session_timeout = if !direct || stream_type == "WebSocket" {
                    RELAXED_SESSION_TIMEOUT
                } else {
                    SEC30
                };
                self.handler
                ...
                let connection_type_label = if direct {
                    format!("Direct {}", stream_type)
                } else {
                    format!("Relay {}", stream_type)
                };
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = self.timer.tick() => {
                            if last_recv_time.elapsed() >= session_timeout {
                        ...
                        _ = status_timer.tick() => {
                            ...
                            let mut speed = std::mem::take(&mut self.data_count);
                            speed = speed * 1000 / elapsed as usize;
                            let speed = format!("{:.2}kB/s", speed as f32 / 1024 as f32);

                            self.quality_status_cache.fps.clear();
                            for (k, v) in self.video_threads.iter() {
                                let fps = (v.frame_count.swap(0, Ordering::Relaxed) as i32)
                                    * 1000
                                    / elapsed as i32;
                                self.quality_status_cache.fps.insert(*k, fps);
                            }
                            let fps_snapshot = self.quality_status_cache.fps.clone();
                            self.fps_control(direct, &fps_snapshot);
                            ...
                            let performance_mode = lc.get_option("performance-mode");
                            // image_quality is a direct PeerConfig field, not in options HashMap
                            let image_quality = lc.image_quality.clone();
                            let requested_fps = lc
                                .get_option("custom-fps")
                                .parse::<u32>()
                                .ok()
                                .filter(|v| *v > 0);
                            let game_mode = performance_mode == "game";
                            ...
                            let decoder_type = self
                                ...
                                .iter()
                                .next()
                                .map(|v| v.1.decoder_type.read().unwrap().clone())
                                .filter(|s| !s.is_empty());
                            let server_qos_fps = self
                            ...
                            self.quality_status_cache.speed = Some(speed);
                            self.quality_status_cache.decode_fps = min_decode_fps;
                            self.quality_status_cache.decoder_type = decoder_type;
                            self.quality_status_cache.performance_mode = Some(performance_mode);
                            self.quality_status_cache.quality = if image_quality.is_empty() {
                                None
                            } else {
                                Some(image_quality)
                            };
                            self.quality_status_cache.requested_fps = requested_fps;
                            self.quality_status_cache.fps_limit = requested_fps;
                            self.quality_status_cache.game_mode = Some(game_mode);
                            // Don't set server_performance_mode/server_game_mode from local value.
                            // These are set by the test_delay handler from the server's response.
                            self.quality_status_cache.server_qos_fps = server_qos_fps;
                            self.quality_status_cache.chroma = chroma;
                            self.quality_status_cache.codec_format = codec_format;
                            self.quality_status_cache.connection_type = Some(connection_type_label.clone());
                            self.handler.update_quality_status(&self.quality_status_cache);
                        }
                    }
                }
        ...
        #[cfg(not(any(target_os = "ios")))]
        {
            ...
            std::thread::spawn(move || {
                loop {
                    ...
                    // Block with short sleep instead of busy-spinning
                    match rx_audio_data.try_recv() {
                        ...
                        Err(_) => {
                            // No data yet, sleep briefly to avoid busy-spinning
        ...
        match data {
                    ...
                    Some(message::Union::Misc(misc)) => match misc.union {
                        Some(misc::Union::RefreshVideo(_)) => {
                            self.video_threads.iter().for_each(|(_, v)| {
                                v.discard_queue.store(true, Ordering::Release);
                            });
                        }
            ...
            Data::FileJobTick => {
                if !self.read_jobs.is_empty() {
                    if let Err(err) = fs::handle_read_jobs(&mut self.read_jobs, peer).await {
                        self.handler
    ...
    fn contains_key_frame(vf: &VideoFrame) -> bool {
        ...
        match &vf.union {
            Some(vf) => match vf {
                H264s(f) | H265s(f) | Av1s(f) | Sccs(f) => f.frames.iter().any(|e| e.key),
                _ => false,
            },
            None => false,
        }
    ...
    fn fps_control(&mut self, direct: bool, real_fps_map: &HashMap<usize, i32>) {
        ...
        let mut limited_fps = if direct {
            min_decode_fps * 19 / 20 // 60 got 57, minimal discount for direct
        } else {
            min_decode_fps * 9 / 10 // 60 got 54, slight discount for relay
        };
        ...
        for (_, thread) in self.video_threads.iter_mut() {
        ...
        let should_increase = !should_decrease && should_increase;
        if last_auto_fps.is_none() || should_decrease || should_increase {
            ...
            if Some(auto_fps) != last_auto_fps {
                ...
                log::info!("Set auto_adjust_fps to {}", auto_fps);
        ...
        for (display, thread) in self.video_threads.iter_mut() {
            let ctl = &mut thread.fps_control;
            let video_queue = &thread.video_queue;
            ...
            match msg_in.union {
                Some(message::Union::VideoFrame(vf)) => {
                    ...
                    if Self::contains_key_frame(&vf) {
                        ...
                        if thread.video_queue.force_push(vf).is_some() {
                            self.handler.refresh_video(display as _);
                        } else {
                ...
                Some(message::Union::TestDelay(t)) => {
                    // Reply to server for RTT measurement
                    if !t.from_client {
                        let mut msg_out = Message::new();
                        msg_out.set_test_delay(t.clone());
                        self.sender.send(Data::Message(msg_out)).ok();
                    }
                    self.handler.update_quality_status(&QualityStatus {
                        delay: Some(t.last_delay as i32),
                        target_bitrate: Some(t.target_bitrate as i32),
                        // Don't let server's performance_mode overwrite local state.
                        // Local performance_mode is set by the client-side timer (io_loop line 360).
                        performance_mode: None,
                        requested_fps: if t.max_custom_fps > 0 {
                            Some(t.max_custom_fps)
                        } else {
                            None
                        },
                        auto_adjust_fps: if t.qos_fps > 0 { Some(t.qos_fps) } else { None },
                        fps_limit: if t.max_custom_fps > 0 {
                            Some(t.max_custom_fps)
                        } else {
                            None
                        },
                        game_mode: None, // use local value from client timer
                        server_performance_mode: if t.performance_mode.is_empty() {
                            None
                        } else {
                            Some(t.performance_mode.clone())
                        },
                        server_game_mode: Some(t.game_mode),
                        server_game_mode_reason: if t.game_mode_reason.is_empty() {
                            None
                        } else {
                            Some(t.game_mode_reason)
                        },
                        server_qos_fps: if t.qos_fps > 0 { Some(t.qos_fps) } else { None },
                        server_max_delay: if t.max_delay > 0 {
                            Some(t.max_delay)
                        } else {
                            None
                        },
                        server_response_delayed: Some(t.response_delayed),
                        server_current_ratio: if t.current_ratio > 0.0 {
                            Some(t.current_ratio)
                        } else {
                            None
                        },
                        server_target_ratio: if t.target_ratio > 0.0 {
                            Some(t.target_ratio)
                        } else {
                            None
                        },
                        server_max_custom_fps: if t.max_custom_fps > 0 {
                            Some(t.max_custom_fps)
                        } else {
                            None
                        },
                        server_abr: Some(t.abr_enabled),
                        ..Default::default()
                    });
                }
    ...
    fn new_video_thread(&mut self, display: usize) {
        let video_queue = Arc::new(ArrayQueue::new(client::VIDEO_QUEUE_SIZE));
        let (video_sender, video_receiver) = std::sync::mpsc::channel::<MediaData>();
        let decode_fps = Arc::new(RwLock::new(None));
        let decoder_type = Arc::new(RwLock::new(String::new()));
        let frame_count = Arc::new(AtomicUsize::new(0));
        let discard_queue = Arc::new(AtomicBool::new(false));
...
struct VideoThread {
    ...
    decoder_type: Arc<RwLock<String>>,
    frame_count: Arc<AtomicUsize>,
    discard_queue: Arc<AtomicBool>,
...
impl Drop for VideoThread {
    fn drop(&mut self) {
        ...
        self.discard_queue.store(true, Ordering::Release);
    }
}
