...
pub fn session_alternative_codecs(session_id: SessionID) -> String {
    if let Some(session) = sessions::get_session_by_session_id(&session_id) {
        let (h264, h265, scc, av1) = session.alternative_codecs();
        let msg = HashMap::from([("h264", h264), ("h265", h265), ("scc", scc), ("av1", av1)]);
    ...
    if key == "is-printer-installed" {
        ...
        if key.starts_with("download-data-") {
            ...
            return match crate::platform::windows::is_msi_installed() {
                Ok(true) => format!("rdesk-{_version}-x86_64.msi"),
                Ok(false) => format!("rdesk-{_version}-x86_64.exe"),
                Err(e) => {
            ...
            #[cfg(target_os = "macos")]
            {
                return if cfg!(target_arch = "x86_64") {
                    format!("rdesk-{_version}-x86_64.dmg")
                } else if cfg!(target_arch = "aarch64") {
                    format!("rdesk-{_version}-aarch64.dmg")
                } else {
