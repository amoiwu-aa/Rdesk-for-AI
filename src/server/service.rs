...
impl<T: Subscriber + From<ConnInner>> Service for ServiceTmpl<T> {
    ...
    fn is_subed(&self, id: i32) -> bool {
        ...
        lock.subscribes.contains_key(&id) || lock.new_subscribes.contains_key(&id)
    }
...
impl<T: Subscriber + From<ConnInner>> ServiceTmpl<T> {
    ...
    pub fn send_video_frame(&self, msg: Message) -> Vec<i32> {
        self.send_video_frame_shared(Arc::new(msg))
    }

    pub fn send_video_frame_shared(&self, msg: Arc<Message>) -> Vec<i32> {
        let mut lock = self.0.write().unwrap();
        let mut conn_ids = Vec::with_capacity(lock.subscribes.len());
        for s in lock.subscribes.values_mut() {
            s.send(msg.clone());
            conn_ids.push(s.id());
        }
