...
pub struct Connection {
    ...
    file_timer: crate::RDeskInterval,
...
impl Connection {
    pub async fn start(
        ...
        let session_timeout = if !direct || matches!(&stream, super::Stream::WebSocket(_)) {
        ...
        loop {
            tokio::select! {
                ...
                Some((instant, value)) = rx_video.recv() => {
                    // Backpressure: drop stale frames if queue is building up
                    let mut latest_instant = instant;
                    let mut latest_value = value;
                    let mut dropped = 0u32;
                    while let Ok((newer_instant, newer_value)) = rx_video.try_recv() {
                        if let Some(message::Union::VideoFrame(_)) = &newer_value.union {
                            dropped += 1;
                            latest_instant = newer_instant;
                            latest_value = newer_value;
                        } else {
                            // Non-video messages (e.g. cursor): send immediately
                            if let Err(err) = conn.stream.send(&newer_value as &Message).await {
                                conn.on_close(&err.to_string(), false).await;
                                break;
                            }
                        }
                    ...
                    if dropped > 0 {
                        log::trace!("Dropped {} stale video frames", dropped);
                    }
                    if !conn.video_ack_required {
                        if let Some(message::Union::VideoFrame(vf)) = &latest_value.union {
                            video_service::notify_video_frame_fetched(vf.display as usize, id, Some(latest_instant.into()));
                        }
                    }
                    if let Err(err) = conn.stream.send(&latest_value as &Message).await {
                        conn.on_close(&err.to_string(), false).await;
                        break;
                    }
                },
                ...
                _ = test_delay_timer.tick() => {
                    if last_recv_time.elapsed() >= conn.session_timeout {
                    ...
                    if conn.last_test_delay.is_none() && !(conn.port_forward_socket.is_some() && conn.authorized) {
                        ...
                        let (
                            ...
                            (
                                qos.bitrate(),
                                qos.fps(),
                                qos.max_delay_value().unwrap_or(conn.network_delay),
                                qos.response_delayed_value(),
                                qos.current_ratio_value(),
                                qos.target_ratio_value(),
                                qos.max_custom_fps_value(),
                                qos.abr_enabled_value(),
                            )
                        };
                        let performance_mode = conn
                            .lr
                            .option
                            .as_ref()
                            .map(|o| o.performance_mode.clone())
                            .unwrap_or_default();
                        ...
                        msg_out.set_test_delay(TestDelay{
                            ...
                            performance_mode,
                            game_mode,
                            game_mode_reason: if game_mode {
                                "已开启实时流媒体极致低延迟调控 (SOTA)。".to_string()
                            } else {
                                "标准模式，但底层 QoS 仍维持 SOTA 级调控。".to_string()
                            },
                            qos_fps,
                            max_delay,
                            response_delayed,
    ...
    fn update_codec_on_login(&self) {
        ...
        if let Some(o) = self.lr.clone().option.as_ref() {
            if let Some(q) = o.supported_decoding.clone().take() {
                ...
                Encoder::update(Update(self.inner.id(), Self::default_supported_decoding()));
            }
        } else {
            Encoder::update(Update(self.inner.id(), Self::default_supported_decoding()));
        }
    ...
    /// Fallback SupportedDecoding for clients that don't report codec abilities.
    /// Assumes H264+H265 support (only supported codecs).
    fn default_supported_decoding() -> SupportedDecoding {
        SupportedDecoding {
            ability_h264: 1,
            ability_h265: 1,
            ability_scc: 1,
            ...
            if t.from_client {
                ...
                if let Some(tm) = self.last_test_delay {
                    ...
                    let game_mode = self
                        ...
                        .unwrap_or_default()
                        == "game";
                    video_service::VIDEO_QOS.lock().unwrap().user_network_delay(
                        self.inner.id(),
                        new_delay,
                        game_mode,
                    );
    ...
    async fn update_options(&mut self, o: &OptionMessage) {
        ...
        let service_name = video_service::get_service_name(self.video_source(), self.display_idx);
        let update_video_service_option = |key: &str, value: &str| {
            if let Some(server) = self.server.upgrade() {
                let lock = server.write().unwrap();
                if let Some(service) = lock.services.get(&service_name) {
                    service.set_option(key, value);
                }
            }
        };
        // SCC debug: log performance_mode reception
        {
            use std::io::Write;
            ...
            {
                let _ = writeln!(f, "[{}] connection.rs: update_options called, performance_mode='{}', service_name='{}'",
                    chrono::Local::now().format("%H:%M:%S"), o.performance_mode, service_name);
                let server_ok = self.server.upgrade().is_some();
                let svc_found = if let Some(server) = self.server.upgrade() {
                    server.read().unwrap().services.contains_key(&service_name)
                } else {
                    false
                };
                let _ = writeln!(f, "  server_ok={}, service_found={}", server_ok, svc_found);
            }
        }
        if !o.performance_mode.is_empty() {
            if let Some(opt) = self.lr.option.as_mut() {
                opt.performance_mode = o.performance_mode.clone();
            } else {
                let mut opt = OptionMessage::new();
                opt.performance_mode = o.performance_mode.clone();
                self.lr.option = Some(opt).into();
            }
            update_video_service_option("performance-mode", &o.performance_mode);
            // SCC debug: confirm option was set
            {
                use std::io::Write;
                if let Ok(mut f) = std::fs::OpenOptions::new()
                    .create(true)
                    ...
                    .open("C:\\ProgramData\\scc_debug.log")
                {
                    let _ = writeln!(
                        f,
                        "  -> set performance-mode='{}' on service '{}'",
                        o.performance_mode, service_name
                    );
                }
            }
        }
        ...
        if !o.performance_mode.is_empty() {
            update_video_service_option(
                "extreme-low-bandwidth",
                if o.performance_mode == "maintenance" {
                    super::service::SERVICE_OPTION_VALUE_TRUE
                } else {
                    super::service::SERVICE_OPTION_VALUE_FALSE
                },
            );
        }
        if o.custom_fps > 0 {
    ...
    fn portable_check(&mut self) {
        ...
        if self.authorized {
            ...
            let uac =
                crate::video_service::IS_UAC_RUNNING.load(std::sync::atomic::Ordering::Relaxed);
            if p.last_uac != uac {
                p.last_uac = uac;
                if !uac || !running {
                    let mut misc = Misc::new();
    ...
    fn update_supported_encoding(&mut self) {
        ...
        if usable.h264 != last.h264 || usable.h265 != last.h265 {
    ...
    async fn send_printer_request(&mut self, data: Vec<u8>) {
        ...
        let path = format!("RDesk://FsJob//Printer/{}", get_time());
