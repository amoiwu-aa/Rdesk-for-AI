    ...
    SimpleCallOnReturn {
        ...
        f: Box::new(move || {
            ...
            std::thread::spawn(move || {
                ...
                if let Ok(mut lock) = SYNC_DISPLAYS.lock() {
                    lock.is_synced = false;
                }
            });
        }),
    }
...
pub(super) fn get_original_resolution(
    ...
    Some(if is_rustdesk_virtual_display {
        ...
        let changed_resolutions = CHANGED_RESOLUTIONS.read().unwrap();
...
pub(super) fn check_update_displays(all: &Vec<Display>) {
    // 1. Fast-Path: Lightweight check to eliminate heavy allocations (Strings, Vecs)
    {
        let lock = SYNC_DISPLAYS.lock().unwrap();
        let is_changed = lock.displays.len() != all.len()
            || all.iter().enumerate().any(|(i, d)| {
                let old = &lock.displays[i];
                #[cfg(target_os = "macos")]
                let scale = d.scale();
                #[cfg(target_os = "linux")]
                let scale = {
                ...
                let scale = 1.0;
                old.width != d.width() as i32
                    || old.height != d.height() as i32
                    || old.x != d.origin().0 as i32
                    || old.y != d.origin().1 as i32
                    || old.online != d.is_online()
                    || old.name != d.name()
                    || (old.scale - scale).abs() > f64::EPSILON
            });
    ...
    let displays = all
        ...
        .map(|d| {
            ...
            let scale = d.scale();
            #[cfg(target_os = "linux")]
            let scale = if use_logical_scale { d.scale() } else { 1.0 };
            #[cfg(not(any(target_os = "macos", target_os = "linux")))]
            let scale = 1.0;
