...
/// Returns the actual binary name (without .exe), which may differ from the display app name.
#[inline]
...
pub fn is_rustdesk() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("RDesk")
}
...
pub async fn get_next_nonkeyexchange_msg(
    ...
    for _i in 0..4 {
        ...
        let result = conn
            .next_timeout(timeout.saturating_sub(elapsed).max(1))
            .await;
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/RDesk.app/Contents/MacOS/RDesk", "XPC_SERVICE_NAME=com.carriez.RDesk_server"
...
pub type RDeskInterval = ThrottledInterval;
...
pub async fn punch_udp(
    ...
    // Send initial port guessing burst (up to +/- 10)
    for i in 0..10 {
        let mut predicted = peer_addr.clone();
        predicted.set_port(peer_addr.port().saturating_add(i as u16));
        socket.send_to(&[], predicted).await.ok();
        
        let mut predicted_down = peer_addr.clone();
        predicted_down.set_port(peer_addr.port().saturating_sub(i as u16));
        socket.send_to(&[], predicted_down).await.ok();
    }
    ...
    loop {
        tokio::select! {
            ...
            res = socket.recv_from(&mut data) => match res {
                ...
                Ok((n, remote_addr)) => {
                    // Ignore random internet noise from unexpected IPs
                    if remote_addr.ip() != peer_addr.ip() {
                        continue;
                    }

                    if listen {
                        if n == 0 {
                            // Do NOT prematurely bind the host socket on empty guessing packets!
                            // Wait for the client to send the actual payload (KCP Handshake).
                            continue;
                        }
                        socket.connect(remote_addr).await.ok();
                        return Ok(Some(bytes::BytesMut::from(&data[..n])));
                    } else {
                        // For the client, receiving any packet from the target IP confirms the path works
                        socket.connect(remote_addr).await.ok();
                        return Ok(None);
                    }
                }
...
mod tests {
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_RDesk_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
