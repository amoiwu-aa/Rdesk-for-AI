...
fn get_before_uninstall(kill_self: bool) -> String {
    let app_name = crate::get_app_name();
    let exe_name = crate::get_exe_name();
    let ext = app_name.to_lowercase();
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging RDesk.
...
pub(super) fn change_resolution_directly(
    ...
    unsafe {
        ...
        // Retrieve current settings first to preserve freq, rotation, bit depth, etc.
        if EnumDisplaySettingsW(device_name.as_ptr(), ENUM_CURRENT_SETTINGS, &mut dm) != 0 {
            has_current = true;
        }
        ...
        let mut res = ChangeDisplaySettingsExW(
            device_name.as_ptr(),
            &mut dm,
            NULL as _,
        ...
        // Fallback: if the new resolution doesn't support the preserved refresh rate or other strict settings,
        // (common when dropping to a low resolution like 1024x768 that doesn't support 144Hz), 
        // fallback to a minimal DEVMODEW request (only Width and Height).
        if res != DISP_CHANGE_SUCCESSFUL && has_current {
            let mut dm_fallback: DEVMODEW = std::mem::zeroed();
            dm_fallback.dmSize = std::mem::size_of::<DEVMODEW>() as _;
            dm_fallback.dmPelsWidth = width as _;
            dm_fallback.dmPelsHeight = height as _;
            dm_fallback.dmFields = DM_PELSHEIGHT | DM_PELSWIDTH;
            
            res = ChangeDisplaySettingsExW(
                device_name.as_ptr(),
                &mut dm_fallback,
                NULL as _,
                CDS_UPDATEREGISTRY | CDS_GLOBAL,
...
pub fn try_kill_rustdesk_main_window_process() -> ResultType<()> {
    ...
    let exe_name = crate::get_exe_name().to_lowercase();
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // name equal
        if !(p_name == exe_name || p_name == exe_name.clone() + ".exe") {
            continue;
        }
        // arg more than 1
        if p.cmd().len() < 1 {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\RDesk\RDesk.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\RDesk\RDesk.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
