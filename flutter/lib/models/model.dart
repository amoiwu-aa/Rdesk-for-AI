...
class FfiModel with ChangeNotifier {
  ...
  handlePlatformAdditions(
    ...
    if (updateData.isEmpty) {
      _pi.platformAdditions.remove(kPlatformAdditionsRDeskVirtualDisplays);
      ...
      try {
        ...
        if (!updateJson
            .containsKey(kPlatformAdditionsRDeskVirtualDisplays)) {
          _pi.platformAdditions
              .remove(kPlatformAdditionsRDeskVirtualDisplays);
        }
...
class QualityMonitorData {
  ...
  String? quality;
  String? requestedFps;
  String? autoAdjustFps;
  String? fpsLimit;
  String? gameMode;
  String? serverPerformanceMode;
  String? serverGameMode;
  String? serverGameModeReason;
  String? serverQosFps;
  String? serverMaxDelay;
  String? serverResponseDelayed;
  String? serverCurrentRatio;
  String? serverTargetRatio;
  String? serverMaxCustomFps;
  String? serverAbr;
  String? delay;
...
class QualityMonitorModel with ChangeNotifier {
  ...
  updateQualityStatus(Map<String, dynamic> evt) {
    try {
      ...
      if (peerId != null && peerId.isNotEmpty) {
        try {
          final mode = PerformanceModeState.find(peerId).value;
          ...
          if (connectionType.isValid()) {
            ...
            _data.connectionType ??= getConnectionText(
              secure,
              direct,
              connectionType.stream_type.value,
            );
          }
          if (mode.isNotEmpty) {
            _data.performanceMode = mode;
            _data.gameMode = (mode == kPerformanceModeGame || mode == kPerformanceModeGameH265 || mode == kPerformanceModeGameAV1) ? '是' : '否';
            if (!_hasRequestedFpsForMode(mode)) {
              _data.requestedFps = _suggestedFpsForMode(mode);
              _data.fpsLimit ??= _data.requestedFps;
            }
            _data.quality ??= _suggestedQualityForMode(mode);
          }
        } catch (_) {
          // ignore state lookup failures for non-remote contexts
        }
      }
      ...
      // performance_mode from event is NOT used here.
      // Local performanceMode is set from PerformanceModeState (line 3432 above).
      // Server's mode is set from 'server_performance_mode' event field (line 3503).
      ...
      if (evt.containsKey('decode_fps') &&
          (evt['decode_fps'] as String).isNotEmpty) {
        _data.decodeFps = evt['decode_fps'];
      }
      if (evt.containsKey('decoder_type') &&
          (evt['decoder_type'] as String).isNotEmpty) {
        _data.decoderType = evt['decoder_type'];
      }
      if (evt.containsKey('quality') && (evt['quality'] as String).isNotEmpty) {
        _data.quality = evt['quality'];
      }
      if (evt.containsKey('requested_fps') &&
          (evt['requested_fps'] as String).isNotEmpty) {
        _data.requestedFps = evt['requested_fps'];
      }
      if (evt.containsKey('auto_adjust_fps') &&
          (evt['auto_adjust_fps'] as String).isNotEmpty) {
        _data.autoAdjustFps = evt['auto_adjust_fps'];
      }
      if (evt.containsKey('fps_limit') &&
          (evt['fps_limit'] as String).isNotEmpty) {
        _data.fpsLimit = evt['fps_limit'];
      }
      // game_mode from event is NOT used here — it can carry stale values.
      // Local gameMode is derived from performanceMode (line 3433 above).
      if (evt.containsKey('server_performance_mode') &&
          (evt['server_performance_mode'] as String).isNotEmpty) {
        _data.serverPerformanceMode = evt['server_performance_mode'];
      }
      if (evt.containsKey('server_game_mode') &&
          (evt['server_game_mode'] as String).isNotEmpty) {
        _data.serverGameMode = evt['server_game_mode'];
      }
      if (evt.containsKey('server_game_mode_reason') &&
          (evt['server_game_mode_reason'] as String).isNotEmpty) {
        _data.serverGameModeReason = evt['server_game_mode_reason'];
      }
      if (evt.containsKey('server_qos_fps') &&
          (evt['server_qos_fps'] as String).isNotEmpty) {
        _data.serverQosFps = evt['server_qos_fps'];
      }
      if (evt.containsKey('server_max_delay') &&
          (evt['server_max_delay'] as String).isNotEmpty) {
        _data.serverMaxDelay = evt['server_max_delay'];
      }
      if (evt.containsKey('server_response_delayed') &&
          (evt['server_response_delayed'] as String).isNotEmpty) {
        _data.serverResponseDelayed = evt['server_response_delayed'];
      }
      if (evt.containsKey('server_current_ratio') &&
          (evt['server_current_ratio'] as String).isNotEmpty) {
        _data.serverCurrentRatio = evt['server_current_ratio'];
      }
      if (evt.containsKey('server_target_ratio') &&
          (evt['server_target_ratio'] as String).isNotEmpty) {
        _data.serverTargetRatio = evt['server_target_ratio'];
      }
      if (evt.containsKey('server_max_custom_fps') &&
          (evt['server_max_custom_fps'] as String).isNotEmpty) {
        _data.serverMaxCustomFps = evt['server_max_custom_fps'];
      }
      if (evt.containsKey('server_abr') &&
          (evt['server_abr'] as String).isNotEmpty) {
        _data.serverAbr = evt['server_abr'];
      }
      if (evt.containsKey('delay') && (evt['delay'] as String).isNotEmpty) {
        _data.delay = evt['delay'];
        _data.serverMaxDelay = evt['delay'];
      }
      ...
      // Only use mode-based default if no explicit quality was set from event
      if (_data.quality == null || _data.quality!.isEmpty) {
        if (_data.performanceMode == kPerformanceModeGame || _data.performanceMode == kPerformanceModeGameH265 || _data.performanceMode == kPerformanceModeGameAV1) {
          _data.quality = 'best';
        } else if (_data.performanceMode == kPerformanceModeStandard) {
          _data.quality = 'balanced';
        } else if (_data.performanceMode == kPerformanceModeOffice) {
          _data.quality = 'best'; // SCC lossless
        } else if (_data.performanceMode == kPerformanceModeMaintenance) {
          _data.quality = 'custom';
        }
      }
      _data.gameMode ??= (_data.performanceMode == kPerformanceModeGame) ? '是' : '否';
      _data.serverGameMode ??= '-';
      _data.serverPerformanceMode ??= '-';
      _data.fpsLimit ??= _data.requestedFps;
      _data.serverQosFps ??= _data.fps;
      _data.serverResponseDelayed ??= 'false';
      _data.serverAbr ??= '-';
      _data.serverGameModeReason ??= _data.performanceMode == kPerformanceModeGame
          ? '当前为游戏模式；更细的 QoS 诊断信息正在恢复中。'
          : '当前为非游戏模式。';
      notifyListeners();
    } catch (e) {
      //
  ...
  bool _hasRequestedFpsForMode(String mode) {
    return _data.requestedFps != null && _data.requestedFps!.trim().isNotEmpty;
  }

  String _suggestedFpsForMode(String mode) {
    switch (mode) {
  ...
  String _suggestedQualityForMode(String mode) {
    switch (mode) {
      case kPerformanceModeGame:
      case kPerformanceModeGameH265:
      case kPerformanceModeGameAV1:
...
class RecordingModel with ChangeNotifier {
  ...
  toggle() async {
    ...
    if (pi == null) return;
    bool value = !_start;
    if (value && !gFFI.userModel.hasMembershipFeature('recording')) {
      final allowed = await gFFI.userModel.ensureMembershipFeature('recording');
      if (!allowed) {
        gFFI.userModel.showMembershipRequiredDialog('录屏');
        return;
      }
    }
    if (value) {
...
class FFI {
  ...
  late final AiOperatorModel aiOperatorModel; // session
  ...
  FFI(SessionID? sId) {
    ...
    aiOperatorModel = AiOperatorModel(WeakReference(this));
...
class PeerInfo with ChangeNotifier {
  ...
  List<int> get RDeskVirtualDisplays => List<int>.from(
      platformAdditions[kPlatformAdditionsRDeskVirtualDisplays] ?? []);
