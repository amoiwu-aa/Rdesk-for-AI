...
class PerformanceModeState {
  static String tag(String id) => 'performance_mode_$id';

  static void init(String id) {
    final key = tag(id);
    if (!Get.isRegistered<RxString>(tag: key)) {
      final RxString state = kPerformanceModeStandard.obs;
      Get.put<RxString>(state, tag: key);
    } else {
      Get.find<RxString>(tag: key).value = kPerformanceModeStandard;
    }
  }
...
removeSharedStates(String id) {
  ...
  PerformanceModeState.delete(id);
