          ...
          builder: (context, qualityMonitorModel, child) => qualityMonitorModel
              ...
              ? Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      _sectionTitle('连接状态'),
                      _row('连接类型', _display(qualityMonitorModel.data.connectionType)),
                      _row('本地性能模式', _display(qualityMonitorModel.data.performanceMode)),
                      _row('请求质量', _display(qualityMonitorModel.data.quality)),
                      _row('速度', _display(qualityMonitorModel.data.speed)),
                      _row(
                          'FPS',
                          "${qualityMonitorModel.data.fps ?? '-'}${qualityMonitorModel.data.decodeFps != null ? ' (D:${qualityMonitorModel.data.decodeFps})' : ''}${qualityMonitorModel.data.decoderType != null ? ' [${qualityMonitorModel.data.decoderType}]' : ''}"),
                      ...
                      _row('编码', _display(qualityMonitorModel.data.codecFormat)),
                      _row('色度', _display(qualityMonitorModel.data.chroma)),
                      _sectionTitle('游戏模式'),
                      _row('本地游戏模式', _display(qualityMonitorModel.data.gameMode)),
                      _row('服务端性能模式',
                          _display(qualityMonitorModel.data.serverPerformanceMode)),
                      _row('服务端游戏模式', _display(qualityMonitorModel.data.serverGameMode)),
                      _row('请求 FPS', _display(qualityMonitorModel.data.requestedFps)),
                      _row('自动调节 FPS', _display(qualityMonitorModel.data.autoAdjustFps)),
                      _row('FPS 上限', _display(qualityMonitorModel.data.fpsLimit)),
                      _row('QoS FPS', _display(qualityMonitorModel.data.serverQosFps)),
                      _row('最高自定义 FPS', _display(qualityMonitorModel.data.serverMaxCustomFps)),
                      _sectionTitle('QoS / 网络'),
                      _row('最大延迟', _display(qualityMonitorModel.data.serverMaxDelay)),
                      _row('响应阻塞', _boolText(qualityMonitorModel.data.serverResponseDelayed)),
                      _row('当前比率', _display(qualityMonitorModel.data.serverCurrentRatio)),
                      _row('目标比率', _display(qualityMonitorModel.data.serverTargetRatio)),
                      _row('ABR', _boolText(qualityMonitorModel.data.serverAbr)),
                      if (_hasValue(qualityMonitorModel.data.serverGameModeReason))
                        Padding(
                          padding: const EdgeInsets.only(top: 8),
                          child: Text(
                            qualityMonitorModel.data.serverGameModeReason!,
                            style: const TextStyle(
