...
class _PeersViewState extends State<_PeersView>
        ...
        builder: (context, snapshot) {
          if (snapshot.hasData) {
            ...
            buildOnePeer(Peer peer, bool isPortrait) {
              ...
              return !isPortrait
                  ? Obx(() => peerCardUiType.value == PeerUiType.list
                      ? SizedBox(height: 72, child: visibilityChild)
                      : peerCardUiType.value == PeerUiType.grid
                           ? SizedBox(
                                width: peerCardGridExtent.value, height: 168, child: visibilityChild)
                           : SizedBox(
                                width: 380, height: 82, child: visibilityChild))
            ...
            final Widget child = Obx(() => stateGlobal.isPortrait.isTrue
                ...
                : _buildLandscapePeersView(peers, buildOnePeer));
  ...
  Widget _buildLandscapePeersView(
    List<Peer> peers,
    ...
    if (peerCardUiType.value == PeerUiType.tile) {
      return ListView.builder(
        controller: _scrollController,
        itemCount: peers.length,
        itemBuilder: (BuildContext context, int index) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        return Obx(() {
          final extent = peerCardGridExtent.value;
          final availableWidth = constraints.maxWidth;
          // Calculate how many columns can fit. Min 1 column.
          int getCrossAxisCount(double extent) {
            int count = ((availableWidth + space) / (extent + space)).floor();
            return count > 0 ? count : 1;
          }
          int crossAxisCount = getCrossAxisCount(extent);
          
          double usedWidth = crossAxisCount * extent + (crossAxisCount - 1) * space;
          
          // If a single column is wider than max constraints, just fill.
          double horizontalPadding = 0;
          if (usedWidth < availableWidth) {
            horizontalPadding = (availableWidth - usedWidth) / 2;
          }
