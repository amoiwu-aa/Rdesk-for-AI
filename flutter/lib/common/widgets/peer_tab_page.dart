...
class _PeerTabPageState extends State<PeerTabPage>
  ...
  void _loadLocalOptions() {
    ...
    final gridExtent = bind.getLocalFlutterOption(k: 'peer-card-grid-extent');
    if (gridExtent != '') {
      peerCardGridExtent.value = double.tryParse(gridExtent) ?? 380.0;
    }
    hideAbTagsPanel.value = false;
  }
    ...
    return ReorderableListView(
        ...
        children: model.visibleEnabledOrderedIndexs.map((t) {
          ...
          final color = selected
              ? MyTheme.accent
              : Theme.of(context).textTheme.bodySmall?.color?.withOpacity(0.72);
          final hover = false.obs;
          final textLabel = t >= 0 && t < _tabLabels.length
              ? _tabLabels[t]
              : model.tabTooltip(t);
          counter += 1;
                     ...
                     child: InkWell(
                       ...
                       child: Container(
                         ...
                         child: Row(
                           ...
                           children: [
                             Icon(model.tabIcon(t), color: color, size: 18),
  ...
  List<Widget> _landscapeRightActions(BuildContext context) {
    ...
    return [
      ...
      const PeerSearchBar().marginOnly(right: 12),
      Container(
        width: 1,
        height: 26,
      ...
      Obx(() => peerCardUiType.value == PeerUiType.grid
          ? SizedBox(
              ...
              child: SliderTheme(
                ...
                child: Slider(
                  value: peerCardGridExtent.value,
                  min: 250,
                  max: 800,
                  onChanged: (v) {
                    peerCardGridExtent.value = v;
                  },
                  onChangeEnd: (v) {
                    bind.setLocalFlutterOption(
                        k: 'peer-card-grid-extent', v: v.toString());
                  },
                ),
              ),
            ).marginOnly(right: 8)
...
class PeerSearchBar extends StatefulWidget {
  ...
  static const double desktopWidth = 250;
...
class _PeerSearchBarState extends State<PeerSearchBar> {
  ...
  void _handleFocusChanged() {
    ...
    if (_focusNode.hasFocus) {
      peerSearchTextController.selection = TextSelection.collapsed(
        offset: peerSearchTextController.value.text.length,
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    if (isDesktop || isWebDesktop) {
      return _buildSearchBar(alwaysVisible: true);
    }
    return drawer
        ? _buildSearchBar()
        : _hoverAction(
  ...
  Widget _buildSearchBar({bool alwaysVisible = false}) {
    return Obx(() => Container(
          width:
              stateGlobal.isPortrait.isTrue ? 132 : PeerSearchBar.desktopWidth,
          height: 40,
          decoration: BoxDecoration(
              ...
              Expanded(
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                          ...
                          hintText:
                              _focused.value ? null : '搜索设备...',
                          hintStyle: TextStyle(
                              fontSize: 14, color: _hintColor(context)),
                    ...
                    // Icon(Icons.close),
                    if (peerSearchTextController.text.isNotEmpty || !alwaysVisible)
                      IconButton(
