...
// File split plan (use `part`/`part of` when ready to compile & test):
// part 'common/common_theme.dart';     // ColorThemeExtension, MyTheme, ParseToString, flatButtonStyle (~520 lines)
// part 'common/common_dialogs.dart';   // Dialog<T>, OverlayDialogManager, CustomAlertDialog, msgBox, showToast (~620 lines)
// part 'common/common_window.dart';    // LastWindowPosition, save/restoreWindowPosition, screen utils (~700 lines)
// part 'common/common_connect.dart';   // URI link handling, connectMainDesktop, connect (~500 lines)
...
class ColorThemeExtension extends ThemeExtension<ColorThemeExtension> {
  ...
  static final light = ColorThemeExtension(
    ...
    border3: Color(0xFF94A3B8),
    highlight: Color(0xFFF1F5F9),
    drag_indicator: Color(0xFF64748B),
    shadow: Color(0x1F000000),
    errorBannerBg: Color(0xFFFEF2F2),
    me: Color(0xFF10B981),
    toastBg: Color(0xCC0F172A),
  ...
  static final dark = ColorThemeExtension(
    ...
    border2: Color(0xFFF1F5F9),
    border3: Color(0x26FFFFFF),
    highlight: Color(0xFF334155),
    drag_indicator: Color(0xFF94A3B8),
    shadow: Color(0x1F000000),
    errorBannerBg: Color(0xFF4C1D95),
    me: Color(0xFF10B981),
    toastBg: Color(0xFFF1F5F9).withOpacity(0.85),
    toastText: Color(0xFF0F172A),
...
class MyTheme {
  ...
  static const Color grayBg = Color(0xFFF8FAFC);
  ...
  static const Color statusNotReady = Color(0xFFE2E8F0);
  static const Color statusError = Color(0xFFEF4444);
  static const Color statusInfo = Color(0xFF3B82F6);
  static const Color statusGray = Color(0xFF9CA3AF);

  // Dark theme surface (SunLogin-style deep dark)
  static const Color darkSurface = Color(0xFF242528);
  static const Color darkScaffold = Color(0xFF1A1B1E);

  // Sidebar / toolbar
  static const Color sidebarColor = Color(0xFF2563EB);
  static const Color borderBlue = Color(0xFF1D4ED8);

  // User avatar colors
  static const Color avatarPurple = Color(0xFFAD448E);
  static const Color avatarTeal = Color(0xFF32bea6);
  static const Color avatarRed = Color(0xFFE04F5F);
  static const Color avatarBlue = Color(0xFF2C8CFF);
  ...
  static ThemeData darkTheme = ThemeData(
    ...
    menuBarTheme: MenuBarThemeData(
        style: MenuStyle(
            backgroundColor: MaterialStatePropertyAll(Color(0xFF0F172A)))),
...
class OverlayDialogManager {
  ...
  void clearMiddleBlocked() {
    if (_overlayKeyState is BlockableOverlayState) {
      (_overlayKeyState as BlockableOverlayState).setMiddleBlocked(false);
    }
  }
...
Color? _msgboxColor(String type) {
  if (type == "input-password" || type == "custom-os-password") {
    return MyTheme.avatarPurple;
  }
  if (type.contains("success")) {
    return MyTheme.avatarTeal;
  }
  if (type.contains("error") || type == "re-input-password") {
    return MyTheme.avatarRed;
  }
  return MyTheme.avatarBlue;
}
...
final Map<String, Color> _str2colorCache = {};

Color str2color(String str, [alpha = 0xFF]) {
  final key = '$str$alpha';
  return _str2colorCache[key] ??= _computeStr2color(str, alpha);
}

Color _computeStr2color(String str, int alpha) {
...
class _PeerPlatformVisualSpec {
  final _PeerPlatformVisualKind kind;
  final String assetName;
  final IconData fallbackIcon;
  final Color color;
  final String label;

  const _PeerPlatformVisualSpec({
    required this.kind,
...
_PeerPlatformVisualSpec _resolvePeerPlatformVisual(String platform) {
  final normalized = platform.trim().toLowerCase();
  if (normalized.isEmpty) {
    return const _PeerPlatformVisualSpec(
      kind: _PeerPlatformVisualKind.genericComputer,
      assetName: 'computer',
  ...
  if (normalized.contains('ubuntu')) {
    return const _PeerPlatformVisualSpec(
      kind: _PeerPlatformVisualKind.ubuntu,
      assetName: 'ubuntu',
      fallbackIcon: Icons.computer_rounded,
  ...
  if (normalized == kPeerPlatformWindows.toLowerCase() ||
      normalized.contains('windows') ||
      normalized.contains('win11') ||
      normalized.contains('win10') ||
      normalized.contains('win7')) {
    return const _PeerPlatformVisualSpec(
      kind: _PeerPlatformVisualKind.windows,
      assetName: 'win',
  ...
  if (normalized == kPeerPlatformAndroid.toLowerCase() ||
      normalized.contains('android')) {
    return const _PeerPlatformVisualSpec(
      kind: _PeerPlatformVisualKind.android,
      assetName: 'android',
  ...
  if (normalized == kPeerPlatformLinux.toLowerCase() ||
      normalized.contains('linux') ||
      normalized.contains('debian') ||
      normalized.contains('fedora') ||
      normalized.contains('centos') ||
      normalized.contains('redhat') ||
      normalized.contains('arch') ||
      normalized.contains('manjaro') ||
      normalized.contains('mint') ||
      normalized.contains('deepin') ||
      normalized.contains('uos') ||
      normalized.contains('kylin') ||
      normalized.contains('elementary') ||
      normalized.contains('zorin') ||
      normalized.contains('pop!')) {
    return const _PeerPlatformVisualSpec(
      kind: _PeerPlatformVisualKind.linux,
      assetName: 'linux',
      fallbackIcon: Icons.desktop_windows_outlined,
      color: Color(0xFF334155),
      label: 'Linux',
    );
  }
...
String describePeerPlatform(String platform) {
  return _resolvePeerPlatformVisual(platform).label;
}

/// Get the image for the current [platform].
Widget getPlatformImage(String platform, {double size = 50}) {
  final spec = _resolvePeerPlatformVisual(platform);
  return SvgPicture.asset(
    'assets/${spec.assetName}.svg',
    height: size,
    width: size,
    fit: BoxFit.contain,
...
connect(BuildContext context, String id,
  ...
  // Extract ID and Password from one-click copy format (ID:Password)
...
Future<void> onActiveWindowChanged() async {
  ...
  if (rustDeskWinManager.getActiveWindows().isEmpty) {
    ...
    try {
      ...
      debugPrint("Start closing RDesk...");
      ...
      if (isMacOS) {
        ...
        // 2024-11-11 11:41:11.546 RDesk[90272:2567686] Failed to create a FlutterPlatformMessageResponseHandle (2)
        // embedder.cc (2672): 'FlutterEngineSendPlatformMessage' returned 'kInvalidArguments'. Invalid engine handle.
        // 2024-11-11 11:41:11.565 RDesk[90272:2567686] Failed to send message to Flutter engine on channel 'flutter/lifecycle' (2).
        // ```
        periodic_immediate(
...
class ServerConfig {
  ...
  ServerConfig.decode(String msg) : name = '' {
    var json = {};
    try {
      // back compatible
      json = jsonDecode(msg);
  ...
  static Future<List<ServerConfig>> loadProfiles() async {
    final s = await bind.mainGetOption(key: 'server-profiles');
    if (s.isEmpty) return [];
    try {
  ...
  static Future<void> saveProfiles(List<ServerConfig> profiles) async {
    ...
    await bind.mainSetOption(key: 'server-profiles', value: json);
  }
}
...
void checkUpdate() {
  // Upgrade check disabled for custom build
}
