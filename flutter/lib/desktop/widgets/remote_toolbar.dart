...
class _ToolbarTheme {
  ...
  static const Color inactiveColor = Color(0xFFF8FAFC);
  static const Color hoverInactiveColor = Color(0xFFE2E8F0);
  ...
  static double menuBorderRadius = isWindows ? 8.0 : 10.0;
  ...
  static Color borderColor(BuildContext context) =>
      Colors.white.withOpacity(0.08); // Subtle border on dark toolbar

  static Color? dividerColor(BuildContext context) =>
      Colors.white.withOpacity(0.2); // Visible divider on dark toolbar

  static MenuStyle defaultMenuStyle(BuildContext context) => MenuStyle(
        backgroundColor: MaterialStatePropertyAll(const Color(0xF02C2C2E)),
...
class _RemoteToolbarState extends State<RemoteToolbar> {
  ...
  Future<void> _openAiAssistantWindow() async {
    if (!await gFFI.userModel.ensureMembershipFeature('ai')) {
      gFFI.userModel.showMembershipRequiredDialog('AI 命令助手');
      return;
    }
    final id = widget.id;
    final connToken = bind.sessionGetConnToken(sessionId: widget.ffi.sessionId);
    await rustDeskWinManager.newAiAssistant(
      id,
      password: null,
          ...
          child: Container(
            ...
            child: ClipRRect(
              ...
              child: BackdropFilter(
                ...
                child: Material(
                  elevation: 0,
                  color: const Color(0xE62C2C2E), // Always dark glass
                  child: Container(
                    decoration: BoxDecoration(
                      border: Border.all(
                        color: Colors.white.withOpacity(0.15),
  ...
  Widget _buildToolbar(BuildContext context) {
    ...
    toolbarItems.add(_ViewMenu(
      id: widget.id, ffi: widget.ffi, state: widget.state, setFullscreen: _setFullscreen));
    toolbarItems.add(_PerformanceMenu(
      id: widget.id, ffi: widget.ffi, state: widget.state));
    if (widget.ffi.connType == ConnType.defaultConn) {
      toolbarItems.add(_ControlMenu(
          id: widget.id, ffi: widget.ffi, state: widget.state));
      toolbarItems.add(_InputMenu(
          id: widget.id, ffi: widget.ffi, state: widget.state));
      toolbarItems.add(_SecurityMenu(
          id: widget.id, ffi: widget.ffi, state: widget.state));
    }
    ...
    toolbarItems.add(_AiAssistantMenu(id: widget.id, ffi: widget.ffi));
        ...
        Container(
          ...
          child: ClipRRect(
            ...
            child: BackdropFilter(
              ...
              child: Material(
                elevation: 0,
                color: const Color(0xE62C2C2E), // Always dark glass for remote toolbar
  ...
  ThemeData themeData() {
    return Theme.of(context).copyWith(
      brightness: Brightness.dark, // Force dark theme for toolbar & menus
      ...
      checkboxTheme: CheckboxThemeData(
        ...
        fillColor: MaterialStateProperty.resolveWith((states) {
          if (states.contains(MaterialState.selected)) {
            return _ToolbarTheme.blueColor;
          }
...
class _MonitorMenu extends StatelessWidget {
  ...
  Widget buildMonitorMenu(BuildContext context) {
    final width = SimpleWrapper<double>(0);
      final monitorsIcon = globalMonitorsWidget(
          context,
          width,
          Colors.white, // Always white on dark toolbar
          Colors.black38);
  ...
  List<Widget> buildMonitorList(BuildContext context, bool isMulti) {
    ...
    // Both toolbar and popups use dark bg → white icons
    final iconColor = Colors.white;
    final inactiveTextColor = _ToolbarTheme.inactiveColor;
    final hoverInactive = _ToolbarTheme.hoverInactiveColor;
    ...
    buildMonitorButton(int i) => Obx(() {
          ...
          if (isAllMonitors) {
            monitorsIcon = globalMonitorsWidget(
                context, width, iconColor, _ToolbarTheme.blueColor,
  ...
  globalMonitorsWidget(BuildContext context, SimpleWrapper<double> width,
      Color activeTextColor, Color activeBgColor, {Color? inactiveColor, Color? borderColor}) {
    getMonitors() {
      final pi = ffi.ffiModel.pi;
      RxInt display = CurrentDisplayState.find(id);
        ...
        children.add(Positioned(
          ...
          child: Container(
            decoration: BoxDecoration(
              ...
              color: display.value == i ? activeBgColor : Colors.transparent,
            ),
            child: Center(
                child: Text(
              '${i + 1}',
              style: TextStyle(
                color: display.value == i
                    ? activeTextColor
                    : (inactiveColor ?? _ToolbarTheme.inactiveColor),
                fontSize: fontSize,
                fontWeight: FontWeight.bold,
              ),
            )),
...
class _DisplayMenuState extends State<_DisplayMenu> {
  ...
  performanceMode() {
    return Obx(() {
      final mode = PerformanceModeState.find(id);
      final sessionId = ffi.sessionId;
      return _SubmenuButton(
        ffi: widget.ffi,
        child: Row(
          children: [
            Text(translate('Performance Mode')),
            const SizedBox(width: 8),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 4, vertical: 1),
              decoration: BoxDecoration(
                color: performanceModeColor(mode.value).withOpacity(0.15),
                borderRadius: BorderRadius.circular(3),
              ),
              child: Text(
                performanceModeLabel(mode.value),
                style: TextStyle(
                  color: performanceModeColor(mode.value),
                  fontSize: 11,
                  fontWeight: FontWeight.bold,
                ),
              ),
        ...
        menuChildren: [
          ...[
            kPerformanceModeMaintenance,
            kPerformanceModeOffice,
            kPerformanceModeStandard,
          ].map((m) => _buildModeRadio(context, m, mode.value, ffi, sessionId, id)),
          // Game sub-menu with H.264 / H.265 / AV1
          SubmenuButton(
            style: _ToolbarTheme.defaultMenuButtonStyle,
            menuStyle: _ToolbarTheme.defaultMenuStyle(context),
            menuChildren: [
              kPerformanceModeGame,
              kPerformanceModeGameH265,
              kPerformanceModeGameAV1,
            ].map((m) => _buildModeRadio(context, m, mode.value, ffi, sessionId, id)).toList(),
            child: Container(
              ...
              child: Row(
                children: [
                  Container(
                    width: 8, height: 8,
                    decoration: BoxDecoration(
                      color: [kPerformanceModeGame, kPerformanceModeGameH265, kPerformanceModeGameAV1]
                          .contains(mode.value) ? performanceModeColor(mode.value) : Colors.red.withOpacity(0.5),
                  ...
                  Text('Game ▸', style: TextStyle(
                    fontSize: 13,
                    fontWeight: [kPerformanceModeGame, kPerformanceModeGameH265, kPerformanceModeGameAV1]
                        .contains(mode.value) ? FontWeight.w600 : FontWeight.w500,
                    color: [kPerformanceModeGame, kPerformanceModeGameH265, kPerformanceModeGameAV1]
                        .contains(mode.value) ? performanceModeColor(mode.value) : Theme.of(context).textTheme.bodyMedium?.color,
                  )),
                ],
              ),
            ),
...
class _RecordMenu extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final recordingAllowed = gFFI.userModel.hasMembershipFeature('recording');
    final visible =
    ...
    return _IconMenuButton(
      ...
      tooltip: recordingModel.start
          ...
          : (recordingAllowed ? 'Start session recording' : 'Recording requires membership'),
      onPressed: () => recordingModel.toggle(),
      color: recordingModel.start
          ? _ToolbarTheme.redColor
          : (recordingAllowed ? _ToolbarTheme.blueColor : Colors.grey),
      hoverColor: recordingModel.start
          ? _ToolbarTheme.hoverRedColor
          : (recordingAllowed ? _ToolbarTheme.hoverBlueColor : Colors.grey),
    );
  }
}

class _AiAssistantMenu extends StatelessWidget {
  final String id;
  final FFI ffi;
  const _AiAssistantMenu({Key? key, required this.id, required this.ffi})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    return _IconMenuButton(
      ...
      tooltip: 'AI 命令助手',
      onPressed: _openAiAssistantWindow,
      color: _ToolbarTheme.blueColor,
      hoverColor: _ToolbarTheme.hoverBlueColor,
    );
  }
...
class _IconMenuButtonState extends State<_IconMenuButton> {
  ...
  Widget build(BuildContext context) {
    ...
    final computedIconColor = (widget.hoverColor == _ToolbarTheme.hoverRedColor || widget.hoverColor == Colors.red) 
                  ? (hover ? Colors.white : Colors.redAccent)
                  : ((widget.assetName == "assets/pinned.svg")
                      ? _ToolbarTheme.blueColor
                      : (hover
                          ? _ToolbarTheme.hoverInactiveColor
                          : _ToolbarTheme.inactiveColor));
    ...
    final icon = widget.icon != null 
        ? IconTheme(data: IconThemeData(color: computedIconColor, size: 20), child: widget.icon!)
        : SizedBox(
            width: 20,
            height: 20,
            child: SvgPicture.asset(
              widget.assetName!,
              colorFilter: ColorFilter.mode(computedIconColor, BlendMode.srcIn),
            ),
          );

    final bgColor = hover
        ? ((widget.hoverColor == _ToolbarTheme.hoverRedColor || widget.hoverColor == Colors.red)
            ? Colors.redAccent
      ...
      child: Container(
        ...
        child: ClipRRect(
          ...
          child: BackdropFilter(
            ...
            child: Material(
              elevation: 0,
              color: const Color(0xE62C2C2E), // Match main toolbar
              child: Container(
                decoration: BoxDecoration(
                  border: Border.all(
                    color: Colors.white.withOpacity(0.15),
...
class _MenuSectionHeader extends StatelessWidget {
  final String title;
  const _MenuSectionHeader({Key? key, required this.title}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Padding(
...
class _ViewMenuState extends State<_ViewMenu> {
  ...
  Widget build(BuildContext context) {
    ...
    menuChildrenGetter(_IconSubmenuButtonState state) {
      final menuChildren = <Widget>[
        const _MenuSectionHeader(title: '屏幕显示比例'),
        _screenAdjustor.adjustWindow(context),
        viewStyle(customPercent: _customPercent),
        
        const _MenuSectionHeader(title: '显示器与滚屏'),
        if (ffi.connType == ConnType.defaultConn)
          _ResolutionsMenu(
            id: widget.id,
            ffi: widget.ffi,
        ...
        scrollStyle(state, colorScheme),
        if (ffi.connType == ConnType.defaultConn && widget.pluginItem is! Offstage) widget.pluginItem,
      ];
      return menuChildren;
    }
    ...
    return _IconSubmenuButton(
      ...
      icon: Container(
        ...
        child: Icon(Icons.desktop_windows_outlined, size: 20, color: Colors.white),
      ),
      ffi: widget.ffi,
      color: _ToolbarTheme.blueColor,
      hoverColor: _ToolbarTheme.hoverBlueColor,
    ...
    return futureBuilder(
        ...
        hasData: (data) {
          ...
          return Column(children: [
            ...v.map((e) {
              final isCustom = e.value == kRemoteViewStyleCustom;
              final child = isCustom ? Text(translate('Scale custom')) : e.child;
              final bool isGroupCustomSelected = e.groupValue == kRemoteViewStyleCustom;
            ...
            if (!isCustomSelected) const Divider(),
            _customControlsIfCustomSelected(
                onChanged: (v) => customPercent.value = v),
          ]);
        });
  ...
  scrollStyle(_IconSubmenuButtonState state, ColorScheme colorScheme) {
    return futureBuilder(future: () async {
      final viewStyle = await bind.sessionGetViewStyle(sessionId: ffi.sessionId) ?? '';
      final visible = viewStyle == kRemoteViewStyleOriginal || viewStyle == kRemoteViewStyleCustom;
      final scrollStyle = await bind.sessionGetScrollStyle(sessionId: ffi.sessionId) ?? '';
      ...
      onChangeScrollStyle(String? value) async {
        if (value == null) return;
        await bind.sessionSetScrollStyle(sessionId: ffi.sessionId, value: value);
        widget.ffi.canvasModel.updateScrollStyle();
        state.setState(() {});
      }
      ...
      onChangeEdgeScrollEdgeThickness(double? value) async {
        if (value == null) return;
        final newThickness = value.round();
        await bind.sessionSetEdgeScrollEdgeThickness(sessionId: ffi.sessionId, value: newThickness);
        widget.ffi.canvasModel.updateEdgeScrollEdgeThickness(newThickness);
        state.setState(() {});
      }
...
// ============== 2. 性能与画质 (_PerformanceMenu) ==============
class _PerformanceMenu extends StatefulWidget {
  final String id;
  final FFI ffi;
  final ToolbarState state;
  _PerformanceMenu({Key? key, required this.id, required this.ffi, required this.state}) : super(key: key);
...
class _PerformanceMenuState extends State<_PerformanceMenu> {
  ...
  Widget buildToggle(String label, String option) {
    return futureBuilder(future: () async {
      return bind.sessionGetToggleOptionSync(sessionId: ffi.sessionId, arg: option);
    }(), hasData: (val) {
      return CkbMenuButton(
          value: val as bool,
          onChanged: (v) async {
            ...
            if (option == "show-quality-monitor") ffi.qualityMonitorModel.checkShowQualityMonitor(ffi.sessionId);
            setState((){});
          },
  ...
  Widget build(BuildContext context) {
    return _IconSubmenuButton(
      ...
      icon: Container(
        alignment: AlignmentDirectional.center,
        constraints: const BoxConstraints(minHeight: _ToolbarTheme.height),
        child: Icon(Icons.speed_rounded, size: 20, color: Colors.white),
      ),
      ffi: widget.ffi,
      color: _ToolbarTheme.blueColor,
      hoverColor: _ToolbarTheme.hoverBlueColor,
      menuChildrenGetter: (_) => [
        const _MenuSectionHeader(title: '性能设定'),
        performanceMode(),
        imageQuality(),
        codec(),
        const Divider(),
        const _MenuSectionHeader(title: '画质与监控'),
        buildToggle('Show quality monitor', 'show-quality-monitor'),
      ],
    );
  }
  ...
  codec() {
    return futureBuilder(
        ...
        hasData: (data) {
          final v = data as List<TRadioMenu<String>>;
          if (v.isEmpty) return const Offstage();
...
// ============== 3. 输入与控制 (_InputMenu) ==============
// Merges the original _KeyboardMenu content with action items
class _InputMenu extends StatelessWidget {
  final String id;
  final FFI ffi;
  final ToolbarState state;
  _InputMenu({Key? key, required this.id, required this.ffi, required this.state}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    // Reuse _KeyboardMenu which has all keyboard/mouse/input items
    return _KeyboardMenu(id: id, ffi: ffi);
  }
}

// ============== 4. 安全与权限 (_SecurityMenu) ==============
class _SecurityMenu extends StatefulWidget {
  final String id;
  final FFI ffi;
  final ToolbarState state;
  _SecurityMenu({Key? key, required this.id, required this.ffi, required this.state}) : super(key: key);

  @override
  State<_SecurityMenu> createState() => _SecurityMenuState();
}

class _SecurityMenuState extends State<_SecurityMenu> {
  FFI get ffi => widget.ffi;
  String get id => widget.id;
  
  Widget buildToggle(String label, String option, {bool disabled = false}) {
    return futureBuilder(future: () async {
      return bind.sessionGetToggleOptionSync(sessionId: ffi.sessionId, arg: option);
    }(), hasData: (val) {
      return CkbMenuButton(
  ...
  Widget build(BuildContext context) {
    return _IconSubmenuButton(
        ...
        icon: Container(
          alignment: AlignmentDirectional.center,
          constraints: const BoxConstraints(minHeight: _ToolbarTheme.height),
          child: Icon(Icons.security_rounded, size: 20, color: Colors.white),
        ),
        color: _ToolbarTheme.blueColor,
        hoverColor: _ToolbarTheme.hoverBlueColor,
        ffi: widget.ffi,
        menuChildrenGetter: (_) {
          final menuChildren = <Widget>[
            const _MenuSectionHeader(title: '剪贴板与音频'),
            buildToggle('Enable file copy and paste', 'enable-file-copy-paste'),
            buildToggle('Disable clipboard', 'disable-clipboard', disabled: ffi.ffiModel.viewOnly),
            buildToggle('Mute', 'disable-audio'),
            const Divider(),
            const _MenuSectionHeader(title: '隐私与安全锁定'),
            buildToggle('Lock after session end', 'lock-after-session-end', disabled: ffi.ffiModel.viewOnly),
          ];
...
// Top-level helper used by both toolbar performance mode menus
Widget _buildModeRadio(BuildContext context, String m, String currentMode, FFI ffi, SessionID sessionId, String id) {
  final isSelected = currentMode == m;
  final mColor = performanceModeColor(m);
  return RdoMenuButton<String>(
    value: m,
    ...
    onChanged: (value) {
      if (value != null) {
        applyPerformanceMode(sessionId, id, value);
      }
    },
    child: Container(
      decoration: BoxDecoration(
        color: isSelected ? mColor.withOpacity(0.12) : Colors.transparent,
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              color: mColor,
              shape: BoxShape.circle,
              boxShadow: [BoxShadow(color: mColor.withOpacity(isSelected ? 0.6 : 0.2), blurRadius: isSelected ? 4 : 2)],
            ),
          ),
          const SizedBox(width: 10),
          Expanded(
            child: Text(
              performanceModeLabel(m),
              style: TextStyle(
                fontSize: 13,
                fontWeight: isSelected ? FontWeight.w600 : FontWeight.w500,
                color: isSelected ? mColor : Theme.of(context).textTheme.bodyMedium?.color,
              ),
            ),
          ),
          if (isSelected) Icon(Icons.check, size: 16, color: mColor) else const SizedBox(width: 16),
        ],
      ),
    ),
  );
