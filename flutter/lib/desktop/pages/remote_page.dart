...
class _RemotePageState extends State<RemotePage>
  ...
  void _handleFileDrop(DropDoneDetails details) {
    ...
    final isRemoteWindows = pi.platform == 'Windows';
    final remoteHome = isRemoteWindows
        ? 'C:\\Users\\${pi.username}\\Desktop'
        : '/home/${pi.username}/Desktop';
    ...
    for (var xfile in details.files) {
      ...
      final remotePath = isRemoteWindows
          ? '$remoteHome\\$fileName'
          : '$remoteHome/$fileName';
      ...
      debugPrint('Drop file transfer: $localPath -> $remotePath');
    }

    // Show toast notification
    showToast(
      '${details.files.length} ${translate("files")} ${translate("sent to")} $remoteHome',
              ...
              ? Container(
                  ...
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        Icon(Icons.file_upload, size: 64, color: Colors.blue.withOpacity(0.8)),
                        const SizedBox(height: 12),
                        Text(
                          translate('Drop files to transfer'),
                          style: TextStyle(
                            fontSize: 20,
...
class _ImagePaintState extends State<ImagePaint> {
  ...
  Widget build(BuildContext context) {
    ...
    final isMaintenanceMode =
        PerformanceModeState.find(id).value == kPerformanceModeMaintenance;
    ...
    if (c.imageOverflow.isTrue && c.scrollStyle != ScrollStyle.scrollauto) {
      ...
      if (c.size.width > 0 && c.size.height > 0) {
        ...
        final content = mouseRegion(child: _buildListener(paintWidget));
        return isMaintenanceMode
            ? ColorFiltered(
                colorFilter: const ColorFilter.matrix(_grayscaleMatrix),
                child: content,
