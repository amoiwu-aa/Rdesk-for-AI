            ...
            child: Obx(() => GestureDetector(
                  ...
                  child: DragItemWidget(
                    dragItemProvider: (DragItemRequest request) {
                      final outItems = selectedItems.items.contains(entry) 
                              ? selectedItems.items 
                              : [entry];
                              
                      final item = DragItem(
                        suggestedName: outItems.first.name,
                        localData: {
                          'isLocal': isLocal,
                          'items': selectedItems.items.contains(entry) 
                              ? selectedItems 
                              : (SelectedItems(isLocal: isLocal)..add(entry)),
                          'controller': controller,
                        },
                      );

                      final outItem = outItems.first;
                      if (isLocal) {
                        for (final localItem in outItems) {
                           item.add(Formats.fileUri(Uri.file(localItem.path)));
                        }
                      } else {
                        // Remote files are dragged out as virtual files! Native drag only supports 1 virtual file per DragItem.
                        // (Internal Flutter transfers will correctly receive all outItems via localData).
                        item.addVirtualFile(
                          format: Formats.plainTextFile, // Changed from DataFormat<Object> returning type map
                          provider: (sinkProvider, progress) {
                            final sink = sinkProvider(fileSize: outItem.size > 0 ? outItem.size : -1);
                            
                            // Request download to local temp directory
                            final remoteController = _ffi.fileModel.remoteController;
                            final localController = _ffi.fileModel.localController;
                            
                            final tempPath = localController.directory.value.path;
                            final destPath = PathUtil.join(tempPath, outItem.name, localController.options.value.isWindows);
                            
                            final items = SelectedItems(isLocal: false)..add(outItem);
                            remoteController.sendFiles(items, localController.directoryData());
                                ...
                                try {
                                  ...
                                  if (await file.exists()) {
                                    ...
                                    if (currentSize > lastReadOffset) {
                                      final readLen = currentSize - lastReadOffset;
                                      final raf = await file.open(mode: FileMode.read);
                                      await raf.setPosition(lastReadOffset);
                                  ...
                                  final jobs = _ffi.fileModel.remoteController.jobController.jobTable;
                                  bool jobStillActive = false;
                                  for (var job in jobs) {
                                    if (job.remote == outItem.path && job.state == JobState.inProgress) {
                                      jobStillActive = true;
                                      break;
                                    }
                                  }
                                  
                                  if (!jobStillActive && lastReadOffset >= outItem.size) {
                                    if (await file.exists()) {
                                      final finalSize = await file.length();
                                      if (finalSize > lastReadOffset) {
                                        final raf = await file.open(mode: FileMode.read);
                                        await raf.setPosition(lastReadOffset);
                                        final bytes = await raf.read(finalSize - lastReadOffset);
                                        await raf.close();
                                        sink.add(bytes);
                                      }
                                    }
                                  ...
                                  debugPrint("Virtual Drag Read Error: $e");
                                }
                              });
                            },
                          );
                    ...
                    dragBuilder: (context, child) {
                      return Material(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: MyTheme.accent, borderRadius: BorderRadius.circular(6), boxShadow: [BoxShadow(color: Colors.black26, blurRadius: 4)]),
                          child: Text(selectedItems.items.contains(entry) && selectedItems.items.length > 1
                              ? "${selectedItems.items.length} 个文件被选中"
                              : entry.name,
          ...
          children: [
            ...
            Expanded(
              child: DropRegion(
                ...
                onDropOver: (event) {
                  try {
                    final rawData = event.session.items.isNotEmpty ? event.session.items.first.localData : null;
                    final data = rawData is Map ? rawData : null;
                    
                    if (data != null && data['isLocal'] != isLocal) {
                ...
                onPerformDrop: (event) async {
                  try {
                    ...
                    if (data != null && data['isLocal'] != isLocal) {
                      final items = data['items'] as SelectedItems;
                      final sourceController = data['controller'] as FileController;
                      sourceController.sendFiles(items, controller.directoryData());
                    } else if (data == null) {
                    final items = SelectedItems(isLocal: false);
                    final otherSideData = _ffi.fileModel.localController.directoryData();
                    int pendingCount = event.session.items.length;
                    
                    void checkAndSend() {
                        pendingCount--;
                        if (pendingCount == 0 && items.items.isNotEmpty) {
                            _ffi.fileModel.remoteController.sendFiles(items, otherSideData);
                        }
                    }
                    ...
                    for (final item in event.session.items) {
                        ...
                        reader.getValue(Formats.fileUri, (Uri? uri) {
                          if (uri != null) {
                            try {
                              ...
                              items.add(Entry()
                                ..path = path
                                ..name = isDir ? path.split(Platform.pathSeparator).last : f.uri.pathSegments.last
                                ..size = isDir ? 0 : f.lengthSync());
                            } catch (e) {
                              debugPrint("Drag parse error: $e");
                            }
                          }
                          checkAndSend();
                        }, onError: (Object? e) {
                      ...
                      debugPrint("onPerformDrop err: $e");
                  }
                },
