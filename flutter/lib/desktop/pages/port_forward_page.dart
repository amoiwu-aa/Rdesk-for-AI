            ...
            if (index == 0) {
              return Container(
                ...
                child: Row(children: [
                  ...
                  text('Remote Port'),
                  SizedBox(
                      width: _kColumn4Width + 60, child: Text(translate('Action')))
                ]),
              );
            } else if (index == 1) {
  ...
  Widget buildTunnelDataRow(BuildContext context, _PortForward pf, int index) {
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        SizedBox(
          width: _kColumn4Width + 60,
          child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceEvenly,
            children: [
              Switch(
                value: bind.mainGetPeerOptionSync(id: widget.id, key: 'pf_disabled_${pf.localPort}') != '1',
                onChanged: (val) async {
                  bind.mainSetPeerOptionSync(id: widget.id, key: 'pf_disabled_${pf.localPort}', value: val ? '0' : '1');
                  if (val) {
                    await bind.sessionAddPortForward(
                        sessionId: _ffi.sessionId, localPort: pf.localPort, remoteHost: pf.remoteHost, remotePort: pf.remotePort);
                  } else {
              ...
              IconButton(
                ...
                onPressed: () async {
                  await bind.sessionAddPortForward(
                      sessionId: _ffi.sessionId, localPort: pf.localPort, remoteHost: '#DELETE#', remotePort: 0);
                  refreshTunnelConfig();
                },
              ),
            ]
