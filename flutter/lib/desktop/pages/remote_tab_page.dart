...
class _ConnectionTabPageState extends State<ConnectionTabPage> {
  ...
  Widget _buildPerformanceModeTag(String peerId) {
    final menuController = MenuController();
    return Obx(() {
      final mode = PerformanceModeState.find(peerId);
      final label = performanceModeLabel(mode.value);
      final color = performanceModeColor(mode.value);
      final remotePage = tabController.state.value.tabs
          .firstWhere((tab) => tab.key == peerId)
          .page as RemotePage;
      final sessionId = remotePage.ffi.sessionId;
      final ffi = remotePage.ffi;
      final gameModes = [kPerformanceModeGame, kPerformanceModeGameH265, kPerformanceModeGameAV1];
      final isGameMode = gameModes.contains(mode.value);

      Widget buildItem(String m) {
        final sel = mode.value == m;
        final c = performanceModeColor(m);
        return MenuItemButton(
          onPressed: () {
            menuController.close();
            applyPerformanceMode(sessionId, peerId, m);
          },
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Text(performanceModeLabel(m), style: TextStyle(fontSize: 13, fontWeight: sel ? FontWeight.w600 : FontWeight.w500, color: sel ? c : null)),
                const SizedBox(width: 8),
                if (sel) Icon(Icons.check, size: 14, color: c),
              ],
            ),
          ),
        );
      ...
      return MenuAnchor(
        ...
        menuChildren: [
          buildItem(kPerformanceModeMaintenance),
          buildItem(kPerformanceModeOffice),
          buildItem(kPerformanceModeStandard),
          SubmenuButton(
            menuChildren: gameModes.map((m) => buildItem(m)).toList(),
            child: Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 4),
              child: Row(
                children: [
                  Container(width: 8, height: 8, decoration: BoxDecoration(
                    color: isGameMode ? performanceModeColor(mode.value) : Colors.red.withOpacity(0.5),
                    shape: BoxShape.circle,
                  )),
                  const SizedBox(width: 10),
                  Text('Game', style: TextStyle(fontSize: 13,
                    fontWeight: isGameMode ? FontWeight.w600 : FontWeight.w500,
                    color: isGameMode ? performanceModeColor(mode.value) : null,
                  )),
                ],
              ),
            ),
...
PopupMenuItem<String> _buildTabModeItem(BuildContext context, String m, String currentMode) {
