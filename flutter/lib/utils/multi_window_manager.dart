...
class RDeskMultiWindowManager {
  RDeskMultiWindowManager._();

  static final instance = RDeskMultiWindowManager._();
  ...
  final List<int> _aiAssistantWindows = List.empty(growable: true);
  ...
  Future<MultiWindowCallResult> newAiAssistant(
    String remoteId, {
    String? password,
    bool? isSharedPassword,
    bool? forceRelay,
    ...
    final params = {
      "type": WindowType.AiAssistant.index,
      "id": remoteId,
      "password": password,
      "forceRelay": forceRelay,
      "isSharedPassword": isSharedPassword,
    ...
    for (final windowId in _aiAssistantWindows.reversed) {
      if (await DesktopMultiWindow.invokeMethod(
          windowId, kWindowEventNewAiAssistant, msg)) {
        return MultiWindowCallResult(windowId, null);
      }
    }

    final windowId = await newSessionWindow(
        WindowType.AiAssistant, remoteId, msg, _aiAssistantWindows, false);
    return MultiWindowCallResult(windowId, null);
  }
...
final rustDeskWinManager = RDeskMultiWindowManager.instance;
